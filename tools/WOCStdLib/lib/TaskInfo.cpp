...
static HRESULT _getProcessMemoryUsage(vm_size_t& virtualSize, vm_size_t& workingSetSize) {
    ...
    virtualSize = (vm_size_t)virtualBytes;
    workingSetSize = (vm_size_t)workingSetBytes;
...
static HRESULT _getProcessCpuUsage(time_value_t& userTime, time_value_t& systemTime) {
    ...
    userTime.seconds = (integer_t)microSeconds / MICROSEC_TO_SEC;
    userTime.microseconds = (integer_t)microSeconds % MICROSEC_TO_SEC;

    microSeconds = kernelTimeSpan.Duration / 10;
    systemTime.seconds = (integer_t)microSeconds / MICROSEC_TO_SEC;
    systemTime.microseconds = (integer_t)microSeconds % MICROSEC_TO_SEC;
