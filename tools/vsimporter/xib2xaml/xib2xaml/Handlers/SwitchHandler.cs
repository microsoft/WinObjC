...
namespace Xib2Xaml.Handlers
{
    internal class SwitchHandler : ViewHandler
    {
        internal new static List<XamlDomObject> CreateObject(XElement tag, XamlSchemaContext schema)
        {
            ...
            // TODO: Investigate new control to replace the legacy Microsoft.Phone.Controls assembly which is not supported under UWP
            //XamlDomObject toggleControlSwitch = new XamlDomObject(schema.GetXamlType(typeof(ToggleControlSwitch)));

            //SetCommonProperties(tag, toggleControlSwitch);
            //domObjects.Add(toggleControlSwitch);
