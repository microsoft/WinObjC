...
protected:
  ...
  llvm::Value *GetClassNamed(CodeGenFunction &CGF,
                             const std::string &Name, bool isWeak, bool isDLLImport) override {
    if (isWeak)
    ...
    if (!ClassSymbol) {
      ClassSymbol = new llvm::GlobalVariable(TheModule, LongTy, false,
                                             ...
                                             nullptr, SymbolName);
      if (isDLLImport) ClassSymbol->setDLLStorageClass(llvm::GlobalValue::DLLImportStorageClass);
	}
...
llvm::Value *CGObjCGNU::GetClass(CodeGenFunction &CGF,
  ...
  return GetClassNamed(CGF, OID->getNameAsString(), OID->isWeakImported(), isDLLImport);
}
...
llvm::Value *CGObjCGNU::EmitIvarOffset(CodeGenFunction &CGF,
  ...
  if (CGM.getLangOpts().ObjCRuntime.isNonFragile()) {
    ...
    if (RuntimeVersion < 10 || CGM.getLangOpts().MSVCCompat)
