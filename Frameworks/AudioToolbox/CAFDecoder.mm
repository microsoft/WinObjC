...
const SInt16    CAFDecoder::sStepTable[89] = {      7,     8,     9,    10,    11,    12,    13,    14,    16,    17,
...
void    CAFDecoder::DecodeChannelSInt16(ChannelState& ioChannelState, UInt32 inNumberChannels, UInt32 inDecodeChannel, UInt32 inNumberPacketsToDecode, const Byte* inInputData, SInt16* outOutputData)
{
...
UInt32  CAFDecoder::ProduceOutputPackets(EbrFile *fpIn, ChannelStateList &mChannelStateList, void* outOutputData, UInt32& ioOutputDataByteSize, UInt32& ioNumberPackets, OutputDescription &mOutputFormat, CAFAudioDescription &mInputFormat)
{
    if ( isPcm ) {
...
bool CAFDecoder::InitForRead(EbrFile *in)
{
    ...
    while ( !EbrFeof(fpIn) && !stop ) {
        ...
        if ( !stop ) EbrFseek(fpIn, (long)chunkEnd, SEEK_SET);
    }
