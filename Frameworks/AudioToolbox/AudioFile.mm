...
public:
    ...
    int readBytes(i64 start, u32* numBytes, void* buffer)
    {
        ...
        *numBytes = 0;
        for (;;) {
            int n = stb_vorbis_get_frame_short_interleaved(_handle, info.channels,
                            data, remainingShorts);
...
public:
    ...
    static AudioFile* openFile(EbrFile *f)
    {
        ...
        while ( !EbrFeof(f) ) {
            ...
            if ( memcmp(dheader.chunkId, "data", 4) == 0 ) break;
...
public:
    ...
    size_t Read(void *dest, size_t elem, size_t count)
    {
        ...
        DWORD amtRead = 0;
        unsigned int status = (unsigned int) EbrCall(readFunc, "dddddd",
            funcContext, 
            position, 0, 
            readCount, 
            dest,
...
DWORD AudioFileOpenWithCallbacks(DWORD context, DWORD readFunc, DWORD writeFunc, DWORD getSizeFunc, DWORD setSizeFunc, DWORD typeHint, AudioFile **out)
{
...
public:
    ...
    ExtAudioFile(AudioFile *pAudio, bool owns) :
        pAudioFile(pAudio),
        ownsAudioFile(owns)
    {
    }
...
OSStatus AudioFileCreateWithURL(CFURLRef url, AudioFileTypeID type, const AudioStreamBasicDescription *format, UInt32 inFlags, AudioFileID *out)
{
...
OSStatus AudioFileReadPacketData(AudioFileID fileID, Boolean useCached, UInt32 *outNumBytes, AudioStreamPacketDescription *packetDescs, SInt64 firstPacket, UInt32 *numPackets, void *outBuf )
{
...
OSStatus AudioFileReadPackets(AudioFileID fileID, Boolean useCached, UInt32 *outNumBytes, AudioStreamPacketDescription *packetDescs, SInt64 firstPacket, UInt32 *numPackets, void *outBuf )
{
    if ( fileID == NULL ) {
        *outNumBytes = 0;
        return 1234;
...
DWORD AudioConverterFillComplexBuffer(DWORD handle, DWORD inProc, DWORD userData, DWORD *outPacketSize, AudioBufferList *outData, AudioStreamBasicDescription *out)
{
    return 0;
}
