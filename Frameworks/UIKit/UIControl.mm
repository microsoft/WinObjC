...
@implementation UIControl : UIView
        ...
        for ( unsigned i = 0; i < count; i ++ ) {
            ...
            if ( (objMask & mask) != 0 ) {
                ...
                if ( curTarget == nil ) {
                    ...
                    while ( curTarget != nil ) {
                        if ( [curTarget respondsToSelector:sel] ) break;
    ...
    -(void) addTarget:(id)target action:(SEL)actionSel forControlEvents:(UIControlEvents)events {
        //EbrDebugLog("%s: addTaret(%s, %s) for 0x%08x\n", object_getClassName(self), target != nil ? object_getClassName(target) : "(nil)", actionSel != NULL ? sel_getName(actionSel) : "(null)", events);
    ...
    -(void) removeTarget:(id)target action:(SEL)actionSel forControlEvents:(UIControlEvents)events {
        ...
        for ( unsigned i = 0; i < count; i ++ ) {
            ...
            if ( target == nil ) {
                ...
                if ( (actionSel == NULL || actionSel == eventSel ) &&
                     (mask & events) != 0 ) {
                    [curEvent invalidate];
                }
            } else {
                ...
                if ( target == [curEvent obj] &&
                    (actionSel == NULL || actionSel == eventSel ) ) {
                    [curEvent invalidate];
                }
            }
        }
