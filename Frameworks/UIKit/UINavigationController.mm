...
@implementation UINavigationController {
    ...
    -(NSArray*) popToRootViewControllerAnimated:(BOOL)animated {
        if ( [_viewControllers count] == 0 ) return [NSArray array];
    ...
    -(void) _showController:(UIViewController*)controller animated:(BOOL)animated {
        if ( _curController == controller ) return;
    ...
    -(void) layoutContainer {
        if ( !_didLayout ) {
            ...
            if ( ![[UIApplication sharedApplication] isStatusBarHidden] &&
                ![_curController wantsFullScreenLayout] && floorf(fabs(containerWindowFrame.origin.y)) == 0.0f ) {
            ...
            if ( _navigationBar != nil ) {
                ...
                if ( isOSTarget(@"7.0") ) {
                    if ( [_newController ? _newController : _curController _edgesForExtendedLayout] == 0 ) { //  [POTENTIAL BUG: Might be a bitmask for top & bottom]
            ...
            if ( _toolBar != nil ) {
                ...
                if ( !_toolBarHidden ) containerRect.size.height -= 45.0f;
