...
@implementation UIPinchGestureRecognizer {
    ...
    static TrackingTouch *findTouch(UIPinchGestureRecognizer *self, UITouch *touch)
    {
        for ( int i = 0; i < self->_numTouches; i ++ ) {
            if ( self->_touches[i].touch == touch ) return &self->_touches[i];
        }
    ...
    -(void) touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
        ...
        if ( _numTouches < 2 ) {
            ...
            _touches[_numTouches]._firstTouchTime = _touches[_numTouches]._prevTouchTime = _touches[_numTouches]._lastTouchTime = [touch timestamp];
