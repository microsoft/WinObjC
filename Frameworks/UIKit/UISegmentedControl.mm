...
@implementation UISegmentedControl : UIControl  {
    ...
    /* annotate with type */ -(id) initWithItems:(id)items {
    ...
    /* annotate with type */ -(id) insertSegment:(DWORD)pos withTitle:(DWORD)title animated:(DWORD)animated {
        assert(0);
        /*
    ...
    static void positionSegments(UISegmentedControl* self)
    {
        ...
        if ( segWidthScaler == 0.0f ) segWidthScaler = 1.0f;
    ...
    /* annotate with type */ -(id) insertSegmentWithTitle:(id)title atIndex:(DWORD)index animated:(DWORD)animated {
    ...
    /* annotate with type */ -(id) imageForSegmentAtIndex:(DWORD)index {
        id segment = [_segments objectAtIndex:index];
        return [segment image];
    }

    /* annotate with type */ -(id) insertSegmentWithImage:(id)image atIndex:(DWORD)index animated:(DWORD)animated {
    ...
    /* annotate with type */ -(id) setTitle:(id)title forSegmentAtIndex:(DWORD)index {
        id segment = [_segments objectAtIndex:index];
    ...
    /* annotate with type */ -(id) setImage:(id)image forSegmentAtIndex:(DWORD)index {
        id segment = [_segments objectAtIndex:index];
    ...
    /* annotate with type */ -(id) removeSegmentAtIndex:(DWORD)index animated:(DWORD)animated {
        id segment = [_segments objectAtIndex:index];
    ...
    /* annotate with type */ -(id) removeAllSegments {
        int count;
    ...
    /* annotate with type */ -(id) titleForSegmentAtIndex:(DWORD)index {
    ...
    /* annotate with type */ -(id) selectSegment:(DWORD)pos {
        if ( pos != _curSelected ) {
    ...
    /* annotate with type */ -(id) setSelectedSegmentIndex:(DWORD)pos {
        if ( pos != _curSelected ) {
            if ( _curSelected != 0xFFFFFFFF ) {
                [[_segments objectAtIndex:_curSelected] _setSelected:0];
            }
            ...
            if ( fireEvent ) [self sendEvent:self mask:UIControlEventValueChanged];
        }

        return self;
    }

    /* annotate with type */ -(id) setSegmentedControlStyle:(DWORD)style {
        return self;
    }

    /* annotate with type */ -(id) setMomentary:(DWORD)isMomentary {
        _isMomentary = isMomentary;
        return self;
    }

    /* annotate with type */ -(id) segmentSelectedDown:(id)segment {
    ...
    /* annotate with type */ -(id) segmentSelectedUp:(id)segment {
        if ( !_isMomentary ) return self;
    ...
    /* annotate with type */ -(id) segmentSelectedCancelled:(id)segment {
        if ( !_isMomentary ) return self;
    ...
    /* annotate with type */ -(id) setTintColor:(id)uiColor {
        _tintColor = uiColor;
    ...
    /* annotate with type */ -(id) setWidth:(float)width forSegmentAtIndex:(int)index {
        id curSegment = [_segments objectAtIndex:index];
    ...
    /* annotate with type */ -(id) setApportionsSegmentWidthsByContent:(BOOL)byContent {
        _apportionsSegmentWidthsByContent = byContent;
        positionSegments(self);
        return self;
    }

    /* annotate with type */ -(id) setDividerImage:(id)img forLeftSegmentState:(int)leftState rightSegmentState:(int)rightState barMetrics:(int)barMetrics {
        int count = [_segments count];
    ...
    /* annotate with type */ -(id) setBackgroundImage:(id)img forState:(NSUInteger)state barMetrics:(NSUInteger)metrics {
    ...
    /* annotate with type */ -(id) setTitleTextAttributes:(id)attributes forState:(NSUInteger)state {
        if ( state == 0 ) {
            int count = [_segments count];
            for ( int i = 0; i < count; i ++ ) {
                id curSegment = [_segments objectAtIndex:i];
    ...
    -(CGSize) sizeThatFits:(CGSize)curSize {
        ...
        if ( ret.width > curSize.width ) ret.width = curSize.width;
        ret.height = 32.0f;
