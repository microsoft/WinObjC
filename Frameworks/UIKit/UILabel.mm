...
@implementation UILabel {
    ...
    -(void) adjustFontSizeToFit {
        ...
        CGRect rect;
        rect = [self bounds];
        if ( rect.size.width == 0.0f || rect.size.height == 0.0f ) return;
    ...
    -(void) adjustTextLayerSize {
        ...
        if ( _isHighlighted && _highlightedTextColor != nil ) color = _highlightedTextColor;
    ...
    -(void) setBackgroundColor:(UIColor*)colorref {
        if (_textLayer != nil) [_textLayer setBackgroundColor:(CGColorRef) colorref];
        if ( !_isHighlighted ) _savedBackgroundColor = colorref;
    ...
    -(CGRect) textRectForBounds:(CGRect)bounds limitedToNumberOfLines:(NSInteger)maxLines {
        ...
        if ( _text != nil ) {
            ...
            size = [_text sizeWithFont:_font constrainedToSize:CGSizeMake(GetCACompositor()->screenWidth(), bounds.size.height) lineBreakMode:_lineBreakMode];
    ...
    -(void) sizeToFit {
        ...
        if ( idealSize.size.width == 0.0f ) {
            ...
            if ( superview != nil ) superSize = [superview bounds];
