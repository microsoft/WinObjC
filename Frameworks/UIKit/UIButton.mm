...
@implementation UIButton {
    ...
    {
        if ( self->_type == UIButtonTypeDetailDisclosure || self->_type == UIButtonTypeInfoLight || self->_type == UIButtonTypeInfoDark || self->_type == UIButtonTypeContactAdd ) {
            return nil;
        }
    ...
    static void setImageProperties(UIButton* self)
    {
        ...
        [[self->_imageView layer] _setContentColor:[UIColor colorWithRed: contentColor.r green: contentColor.g blue: contentColor.b alpha: contentColor.a]];
    }
    ...
    -(id) initWithCoder:(NSCoder*)coder {
        ...
        if ( [coder containsValueForKey:@"UIBackgroundColor"] ) {
            ...
            if (type == UIButtonTypeRoundedRect || type == UIButtonTypeRoundedRectLegacy ) {  // We're not sure about roundedRect, feel free to delete if needed.
        ...
        if ( stateContent != nil ) {
            ...
            while ( curKey != nil ) {
                ...
                if ( image != nil ) _states[stateNum].image = image;
                ...
                if ( backgroundImage != nil ) _states[stateNum].backgroundImage = backgroundImage;
    ...
    -(instancetype) initWithFrame:(CGRect)pos {
        ...
        [_label setAutoresizingMask:UIViewAutoresizingFlexibleLeftMargin | UIViewAutoresizingFlexibleRightMargin | UIViewAutoresizingFlexibleTopMargin | UIViewAutoresizingFlexibleBottomMargin];
    ...
    -(void) drawRect:(CGRect)inRect {
        ...
        bounds = [self bounds];
        if ( getBackgroundImage(self) != nil ) [getBackgroundImage(self) drawInRect:bounds];
    }
#endif
    ...
    -(void) touchesBegan:(NSSet*)touchSet withEvent:(UIEvent*)event {
        if ( _curState & UIControlStateDisabled ) return;
    ...
    static CGRect calcImageRect(UIButton* self, CGRect bounds)
    {
        ...
        if ( ret.size.width > imageSize.width ) ret.size.width = imageSize.width;
        if ( ret.size.height > imageSize.height ) ret.size.height = imageSize.height;
