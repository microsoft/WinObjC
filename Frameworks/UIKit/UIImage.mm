...
void UIImageSetLayerContents(CALayer* layer, UIImage* image)
{
    ...
    if ( cgImage != curImage ) {
        ...
        if ( stretch.origin.x != 0.0f || stretch.size.width != 1.0f ||
             stretch.origin.y != 0.0f || stretch.size.height != 1.0f ) {
...
@implementation UIImage {
    ...
    +(UIImage*) applicationImageNamed:(NSString*)pathAddr {
        ...
        if (found == nil) {
            ...
            float scale = 1.f;
            if ( strstr([pathAddr UTF8String], "@2x") != NULL ) scale = 2.0f;
            if (fileData) {
    ...
    /* annotate with type */ +(id) imageWithCGImage:(CGImageRef)image {
        return [[[self alloc] initWithCGImage:image] autorelease];
    }

    /* annotate with type */ +(id) imageWithCGImage:(CGImageRef)image scale:(float)scaleFactor orientation:(UIImageOrientation)orientation {
    ...
    -(instancetype) initWithContentsOfFile:(NSString*)pathAddr {
        ...
        if ( strrchr(path, '.') != NULL && GetCACompositor()->screenScale() > 1.5f ) {
            ...
            if ( EbrAccess(pathStr, 0) == -1 ) {
                id pathFind = [bundle pathForResource:[NSString stringWithCString: newStr] ofType:nil inDirectory:nil forLocalization:@"English"];
        ...
        if ( in[0] == 0x89 && in[1] == 'P' && in[2] == 'N' && in[3] == 'G'  ) {
            ...
            if ( ((CGJPEGImageBacking *) m_pImage->Backing()) && 
                 ((CGJPEGImageBacking *) m_pImage->Backing())->_orientation ) {
    ...
    static void drawNinePatchCallBack(void *info, CGContextRef context)
    {
        ...
        if(img->_imageInsets.top) {     
            ...
            if(getImage(img)->Backing()->Width() - img->_imageInsets.left - img->_imageInsets.right - 2 > 0) {
                CGContextDrawImageRect(context, getImage(img), 
                    makeRect(img->_imageInsets.left + 1, getImage(img)->Backing()->Height() - img->_imageInsets.top, getImage(img)->Backing()->Width() - img->_imageInsets.left - img->_imageInsets.right - 2, img->_imageInsets.top), 
        ...
        if(getImage(img)->Backing()->Height() - img->_imageInsets.top - img->_imageInsets.bottom > 0) {
            ...
            if(getImage(img)->Backing()->Width() - img->_imageInsets.left - img->_imageInsets.right - 2 > 0) {
                CGContextDrawImageRect(context, getImage(img), 
                    makeRect(img->_imageInsets.left + 1, img->_imageInsets.bottom, getImage(img)->Backing()->Width() - img->_imageInsets.left - img->_imageInsets.right-2, getImage(img)->Backing()->Height() - img->_imageInsets.top - img->_imageInsets.bottom), 
        ...
        if(img->_imageInsets.bottom) {      
            ...
            if(getImage(img)->Backing()->Width() - img->_imageInsets.left - img->_imageInsets.right - 2 > 0) {
                CGContextDrawImageRect(context, getImage(img), 
                    makeRect(img->_imageInsets.left + 1, 0, getImage(img)->Backing()->Width() - img->_imageInsets.left - img->_imageInsets.right - 2, img->_imageInsets.bottom), 
    ...
    static void drawLeftCap(UIImage* self, CGContextRef cur, CGRect pos)
    {
        ...
        CGContextDrawImageRect(cur, getImage(self), 
            capRect, 
            drawRect);
    ...
    static void drawLeftAndTopCap(UIImage* self, CGContextRef ctx, CGRect rect)
    {
        ...
        //topLeftCorner
        drawFromRect(self, ctx, makeRect(CGRectGetMinX(rect), CGRectGetMinY(rect), self->_imageInsets.left, self->_imageInsets.top), makeRect(0, 0, self->_imageInsets.left, self->_imageInsets.top));

        //topEdgeFill
        drawFromRect(self, ctx, makeRect(CGRectGetMinX(rect) + self->_imageInsets.left, CGRectGetMinY(rect), rect.size.width - rightCapWidth - self->_imageInsets.left, self->_imageInsets.top),
            makeRect(self->_imageInsets.left, 0, stretchyWidth, self->_imageInsets.top));

        //topRightCorner
        drawFromRect(self, ctx, makeRect(CGRectGetMaxX(rect)-rightCapWidth, CGRectGetMinY(rect), rightCapWidth, self->_imageInsets.top),
           makeRect(size.width-rightCapWidth, 0, rightCapWidth, self->_imageInsets.top));
    
        //bottomLeftCorner
        drawFromRect(self, ctx, makeRect(CGRectGetMinX(rect), CGRectGetMaxY(rect) - bottomCapHeight, self->_imageInsets.left, bottomCapHeight),
           makeRect(0, size.height - bottomCapHeight, self->_imageInsets.left, bottomCapHeight));
    
        //bottomEdgeFill
        drawFromRect(self, ctx, makeRect(CGRectGetMinX(rect) + self->_imageInsets.left, CGRectGetMaxY(rect) - bottomCapHeight, rect.size.width - rightCapWidth - self->_imageInsets.left, bottomCapHeight),
           makeRect(self->_imageInsets.left, size.height - bottomCapHeight, stretchyWidth, bottomCapHeight));
    
        //bottomRightCorner
        drawFromRect(self, ctx, makeRect(CGRectGetMaxX(rect) - rightCapWidth, CGRectGetMaxY(rect) - bottomCapHeight, rightCapWidth, bottomCapHeight),
           makeRect(size.width - rightCapWidth, size.height - bottomCapHeight, rightCapWidth, bottomCapHeight));
    
        //leftEdgeFill
        drawFromRect(self, ctx, makeRect(CGRectGetMinX(rect), CGRectGetMinY(rect) + self->_imageInsets.top, self->_imageInsets.left, rect.size.height - bottomCapHeight - self->_imageInsets.top),
           makeRect(0, self->_imageInsets.top, self->_imageInsets.left, stretchyHeight));
    
        //rightEdgeFill
        drawFromRect(self, ctx, makeRect(CGRectGetMaxX(rect) - rightCapWidth, CGRectGetMinY(rect) + self->_imageInsets.top, rightCapWidth, rect.size.height - bottomCapHeight - self->_imageInsets.top),
           makeRect(size.width - rightCapWidth, self->_imageInsets.top, rightCapWidth, stretchyHeight));
    
        //centerFill
        drawFromRect(self, ctx, makeRect(CGRectGetMinX(rect) + self->_imageInsets.left, CGRectGetMinY(rect) + self->_imageInsets.top, rect.size.width - rightCapWidth - self->_imageInsets.left, rect.size.height - bottomCapHeight - self->_imageInsets.top),
    ...
    -(void) drawInRect:(CGRect)pos blendMode:(CGBlendMode)mode alpha:(float)alpha {
        ...
        if ( _imageInsets.left == 0 && _imageInsets.top == 0 && _imageInsets.right == 0 && _imageInsets.bottom == 0) {
            ...
            CGPatternRef pat = CGPatternCreate((void*)&ii, makeRect(0, 0, pos.size.width * _scale, pos.size.height * _scale), CGAffineTransformMakeScale(_scale, _scale), 10, 10, kCGPatternTilingConstantSpacing, true, &patCB);
    ...
    -(void) dealloc {
        ...
        if ( m_pImage ) CGImageRelease(m_pImage);
        [super dealloc];
    }
