...
@public
    ...
    -(UIViewController*) selectedViewController {
        if ( _selectedIndex == -1 ) return nil;
    ...
    -(void) loadView {
        if ( _moreNavigationController == nil ) _moreNavigationController = [UITabMoreController new];
        if ( [self nibName] != nil ) {
            [super loadView];
        }
    ...
    -(void) _tabBar:(UITabBar*)tabBar didSelectItem:(UITabBarItem*)item {
        ...
        if ( ![_delegate respondsToSelector:@selector(tabBarController:shouldSelectViewController:)] || [_delegate tabBarController:self shouldSelectViewController:controller] ) {
    ...
    -(void) notifyViewWillAppear:(BOOL)animated {
        if ( _mainView != nil ) [((UITabPane*) _tabPane)->_curController notifyViewWillAppear:animated];

        [super notifyViewWillAppear:animated];
    }

    -(void) notifyViewDidAppear:(BOOL)isAnimated {
        if ( _mainView != nil ) [((UITabPane*) _tabPane)->_curController notifyViewDidAppear:isAnimated];

        [super notifyViewDidAppear:isAnimated];
    }

    -(void) notifyViewWillDisappear:(BOOL)isAnimated {
        if ( _mainView != nil ) [((UITabPane*) _tabPane)->_curController notifyViewWillDisappear:isAnimated];

        [super notifyViewWillDisappear:isAnimated];
    }

    -(void) notifyViewDidDisappear:(BOOL)isAnimated {
        if ( _mainView != nil ) [((UITabPane*) _tabPane)->_curController notifyViewDidDisappear:isAnimated];
