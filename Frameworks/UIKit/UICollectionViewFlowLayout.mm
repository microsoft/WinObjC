...
@implementation UICollectionViewFlowLayout {
    ...
    /* annotate with type */ -(id) init {
        ...
        [_rowAlignmentsOptionsDictionary setObject:[NSNumber numberWithInt:UIFlowLayoutHorizontalAlignmentJustify] forKey:@"UIFlowLayoutCommonRowHorizontalAlignmentKey"];
        [_rowAlignmentsOptionsDictionary setObject:[NSNumber numberWithInt:UIFlowLayoutHorizontalAlignmentJustify] forKey:@"UIFlowLayoutLastRowHorizontalAlignmentKey"];
        [_rowAlignmentsOptionsDictionary setObject:[NSNumber numberWithInt:1] forKey:@"UIFlowLayoutRowVerticalAlignmentKey"];
        return self;
    }

    /* annotate with type */ -(id) initWithCoder:(id)decoder {
    ...
    /* annotate with type */ -(void) setScrollDirection:(UICollectionViewScrollDirection)direction {
        _scrollDirection = direction;
    }

    /* annotate with type */ -(void) setItemSize:(CGSize)size {
        _itemSize = size;
    }
    ...
    /* annotate with type */ -(id) setHeaderReferenceSize:(CGSize)size {
        return self;
    }

    /* annotate with type */ -(id) setFooterReferenceSize:(CGSize)size {
        return self;
    }

    /* annotate with type */ -(void) setMinimumInteritemSpacing:(float)spacing {
    ...
    /* annotate with type */ -(void) setMinimumLineSpacing:(float)spacing {
    ...
    /* annotate with type */ -(id) fetchItemsInfo {
        [self getSizingInfos];
    ...
    /* annotate with type */ -(id) updateItemsLayout {
        ...
        for (UIGridLayoutSection* section in [_data sections]) {
            ...
            if ( [_data horizontal]) {
                ...
                contentSize.height = std::max(contentSize.height, origSectionFrame.size.height + origSectionFrame.origin.y + sectionMargins.top + sectionMargins.bottom);
            }else {
                sectionFrame.origin.y += contentSize.height;
                contentSize.height += origSectionFrame.size.height + origSectionFrame.origin.y;
                contentSize.width = std::max(contentSize.width, origSectionFrame.size.width + origSectionFrame.origin.x + sectionMargins.left + sectionMargins.right);
            }
    ...
    /* annotate with type */ -(id) getSizingInfos {
        ...
        for (NSUInteger section = 0; section < numberOfSections; section++) {
            ...
            if ( [flowDataSource respondsToSelector:@selector(collectionView:layout:minimumLineSpacingForSectionAtIndex:)]) {
                CGFloat minimumLineSpacing = [flowDataSource collectionView:_collectionView layout:self minimumLineSpacingForSectionAtIndex:section];
                if ( [_data horizontal]) {
            ...
            if ( [flowDataSource respondsToSelector:@selector(collectionView:layout:minimumInteritemSpacingForSectionAtIndex:)]) {
                CGFloat minimumInterimSpacing = [flowDataSource collectionView:_collectionView layout:self minimumInteritemSpacingForSectionAtIndex:section];
                if ( [_data horizontal] ) {
    ...
    /* annotate with type */ -(id) layoutAttributesForItemAtIndexPath:(id)indexPath {
        return [self _buildLayoutAttributesForItemAtIndexPath:indexPath];
    }

    /* annotate with type */ -(id) _buildLayoutAttributesForItemAtIndexPath:(NSIndexPath*)indexPath {
        if (!_data) [self prepareLayout];
    ...
    /* annotate with type */ -(id) _buildLayoutAttributesForElementsInRect:(CGRect)rect {
        ...
        for (UIGridLayoutSection* section in [_data sections]) {
            ...
            if (CGRectIntersectsRect(frame, rect)) {
                ...
                if (!CGRectIsEmpty(normalizedHeaderFrame) && CGRectIntersectsRect(normalizedHeaderFrame, rect)) {
                    id layoutAttributes = [[[self class] layoutAttributesClass] layoutAttributesForSupplementaryViewOfKind:@"UICollectionElementKindSectionHeader" withIndexPath:[NSIndexPath indexPathForItem:0 inSection:sectionIndex]];
                ...
                if (!itemRects && [section fixedItemSize] && [[section rows] count]) {
                    ...
                    if (itemRects) [rectCache setObject:itemRects forKey:[NSNumber numberWithInt:sectionIndex]];
                }
                ...
                for (UIGridLayoutRow* row in [section rows]) {
                    ...
                    if (CGRectIntersectsRect(normalizedRowFrame, rect)) {
                        ...
                        for (unsigned itemIndex = 0; itemIndex < [row itemCount]; itemIndex++) {
                            ...
                            CGRect itemRect = CGRectMake(normalizedRowFrame.origin.x + itemFrame.origin.x, normalizedRowFrame.origin.y + itemFrame.origin.y, itemFrame.size.width, itemFrame.size.height);
                ...
                if (!CGRectIsEmpty(normalizedFooterFrame) && CGRectIntersectsRect(normalizedFooterFrame, rect)) {
                    id layoutAttributes = [[[self class] layoutAttributesClass] layoutAttributesForSupplementaryViewOfKind:@"UICollectionElementKindSectionFooter" withIndexPath:[NSIndexPath indexPathForItem:0 inSection:sectionIndex]];
    ...
    /* annotate with type */ -(id) layoutAttributesForSupplementaryViewOfKind:(id)kind atIndexPath:(NSIndexPath*)indexPath {
        if (!_data) [self prepareLayout];
