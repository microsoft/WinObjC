...
enum TableViewNodeType { tableViewNodeRow, tableViewNodeExtra, tableViewNodeUnknown };
...
public:
    ...
    bool isInVisibleRange(CGRect& rect, BOOL animated) {
        if (!animated) {
            if (_height == 0.0f)
                return false;
            if (_yPos + _height < rect.origin.y || _yPos > rect.origin.y + rect.size.height) {
                return false;
            } else {
                return true;
            }
            ...
            if (_addedToView) {
                if (_oldYPos + _oldHeight >= rect.origin.y && _oldYPos <= rect.origin.y + rect.size.height) {
                    ret = true;
                }
            } else {
                if (_height == 0.0f)
...
public:
    ...
    bool isSectionVisible(CGRect& rect) {
        if (_yPos + _sectionHeight < rect.origin.y || _yPos > rect.origin.y + rect.size.height) {
            return false;
        } else {
            return true;
        }
...
public:
    ...
    virtual void nodeInvisible() {
        if (!_reusable) {
            if (_view != nil) {
                if (![_curFirstResponder respondsToSelector:@selector(isDescendantOfView:)] ||
                    ![_curFirstResponder isDescendantOfView:_view]) {
...
static void calcCellPositions(UITableView* self) {
    ...
    if (priv->_style == UITableViewStyleGrouped)
        y += 10.0f;
...
- (NSIndexPath*)indexPathForRowAtPoint:(CGPoint)point {
    ...
    LLTREE_FOREACH(curNode, tablePriv->_rootNode) {
        ...
        LLTREE_FOREACH(curRowNode, ((TableViewNode*)curSection)) {
            ...
            if (curRow->_yPos + curRow->_height > point.y && curRow->_yPos <= point.y) {
...
- (NSIndexPath*)indexPathsForRowsInRect:(CGRect)rect {
    ...
    LLTREE_FOREACH(curNode, tablePriv->_rootNode) {
        ...
        LLTREE_FOREACH(curRowNode, ((TableViewNode*)curSection)) {
            ...
            if (curRow->_yPos + curRow->_height > rect.origin.y && curRow->_yPos <= rect.origin.y + rect.size.height) {
                id index = [NSIndexPath indexPathForRow:curRowIndex inSection:curSectionIndex];
                [ret addObject:index];
            }
...
- (void)setBackgroundView:(UIView*)view {
    ...
    if (tablePriv->_backgroundView != nil)
        [tablePriv->_backgroundView removeFromSuperview];
