...
@implementation UIAlertView {
    ...
    /* annotate with type */ -(id) initWithTitle:(NSString*)title message:(NSString*)message delegate:(id)delegate cancelButtonTitle:(NSString*)cancelButtonTitle otherButtonTitles:(NSString*)otherButtonTitles , ...{
        va_list pReader; va_start(pReader, otherButtonTitles);
        [self setDelegate:delegate];
        if ( title != nil ) alertPriv->_title.attach([title copy]);
        if ( message != nil ) alertPriv->_message.attach([message copy]);
    ...
    /* annotate with type */ -(id) initWithTitle:(id)title message:(id)message delegate:(id)delegate defaultButton:(id)defaultButton cancelButton:(id)cancelButton otherButtons:(id)otherButtons {
        [self setDelegate:delegate];
        if ( title != nil ) alertPriv->_title.attach([title copy]);
        if ( message != nil ) alertPriv->_message.attach([message copy]);
    ...
    /* annotate with type */ -(void) setTitle:(id)title {
    ...
    /* annotate with type */ -(void) setMessage:(id)message {
    ...
    /* annotate with type */ -(id) setNumberOfRows:(DWORD)numRows {
    ...
    /* annotate with type */ -(void) setCancelButtonIndex:(int)index {
    ...
    /* annotate with type */ -(void) dismissWithClickedButtonIndex:(int)index animated:(BOOL)animated {
        [self retain];
        [self autorelease];
    ...
    -(void) /* use typed version */ show {
        ...
        if ( alertPriv->_title != nil ) {
            titleSize = [alertPriv->_title sizeWithFont:titleFont constrainedToSize:CGSizeMake(itemWidth, 480.0f) lineBreakMode:UILineBreakModeWordWrap];
        ...
        if ( alertPriv->_message != nil ) {
            ...
            for ( ;; ) {
                messageSize = [alertPriv->_message sizeWithFont:messageFont constrainedToSize:CGSizeMake(itemWidth, 480.0f) lineBreakMode:UILineBreakModeWordWrap];
                if ( messageSize.height < maxHeight ) break;

                float curSize = [messageFont pointSize];
                curSize -= 0.5f;
                messageFont = [messageFont fontWithSize:curSize];
                if ( curSize <= 1.0f ) break;
            } 

            if ( messageSize.height > maxHeight ) messageSize.height = maxHeight;
        ...
        if ( alertPriv->_cancelText != nil && alertPriv->_numButtons == 1 && alertPriv->_numberOfRows < 2 ) {
            ...
            alertPriv->_cancelView = (createButton(self, alertPriv->_cancelButtonIndex, alertPriv->_cancelText, boxWidth / 2.0f - buttonsWidth / 2.0f, curHeight, 130.0f));
            [alertPriv->_cancelView sendControlEventsOnBack:UIControlEventTouchUpInside];

            for ( int i = 0; i < alertPriv->_numButtons; i ++ ) {
                alertPriv->_buttons[i]._buttonView.attach(createButton(self, i + alertPriv->_otherButtonIndex, alertPriv->_buttons[i]._buttonText, boxWidth / 2.0f - buttonsWidth / 2.0f + 140.0f, curHeight, 130.0f));
            }
            ...
            for ( int i = 0; i < alertPriv->_numButtons; i ++ ) {
                alertPriv->_buttons[i]._buttonView.attach(createButton(self, i + alertPriv->_otherButtonIndex, alertPriv->_buttons[i]._buttonText, boxWidth / 2.0f - 280.0f / 2.0f, curHeight, 280.0f));
                curHeight += 40.0f;
            }

            if ( alertPriv->_cancelText != nil ) {
                curHeight += 10.0f;
                alertPriv->_cancelView = (createButton(self, alertPriv->_cancelButtonIndex, alertPriv->_cancelText, boxWidth / 2.0f - 280.0f / 2.0f, curHeight, 280.0f));
                [alertPriv->_cancelView sendControlEventsOnBack:UIControlEventTouchUpInside];
        ...
        if ( alertPriv->_messageLabel ) [self addSubview:(id) alertPriv->_messageLabel];
        if ( alertPriv->_titleLabel ) [self addSubview:(id) alertPriv->_titleLabel];

        if ( alertPriv->_cancelView ) [self addSubview:(id) alertPriv->_cancelView];
        for ( int i = 0; i < alertPriv->_numButtons; i ++ ) {
        ...
        [self setAutoresizingMask:UIViewAutoresizingFlexibleLeftMargin | UIViewAutoresizingFlexibleRightMargin | UIViewAutoresizingFlexibleBottomMargin | UIViewAutoresizingFlexibleTopMargin];
        [self setAlpha:0.0f];
    ...
    /* annotate with type */ -(id) buttonTitleAtIndex:(int)index {
        if ( index == alertPriv->_cancelButtonIndex ) return alertPriv->_cancelText;
    ...
    /* annotate with type */ -(id) textFieldAtIndex:(int)index {
