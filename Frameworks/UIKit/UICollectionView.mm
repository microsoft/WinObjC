...
@implementation UICollectionView {
    ...
    /* annotate with type */ -(id) initWithCoder:(id)inCoder {
    ...
    /* annotate with type */ -(void) awakeFromNib {
        [super awakeFromNib];
    ...
    /* annotate with type */ -(id) initWithFrame:(CGRect)pos collectionViewLayout:(id)layout {
    ...
    /* annotate with type */ -(id) initWithFrame:(CGRect)pos {
        return [self initWithFrame:pos collectionViewLayout:nil];
    }

    /* annotate with type */ -(void) setDataSource:(id)dataSource {
    ...
    /* annotate with type */ -(id) dataSource {
        return _dataSource;
    }
    ...
    /* annotate with type */ -(void) setDelegate:(id)delegate {
        ...
        _collectionViewFlags.delegateShouldHighlightItemAtIndexPath = [delegate respondsToSelector:@selector(collectionView:shouldHighlightItemAtIndexPath:)];
        _collectionViewFlags.delegateDidHighlightItemAtIndexPath = [delegate respondsToSelector:@selector(collectionView:didHighlightItemAtIndexPath:)];
        _collectionViewFlags.delegateDidUnhighlightItemAtIndexPath = [delegate respondsToSelector:@selector(collectionView:didUnhighlightItemAtIndexPath:)];
        ...
        _collectionViewFlags.delegateDidEndDisplayingSupplementaryView = [delegate respondsToSelector:@selector(collectionView:didEndDisplayingSupplementaryView:forElementOfKind:atIndexPath:)];
    ...
    /* annotate with type */ -(id) setCollectionViewLayout:(id)layout animated:(BOOL)animated {
        if (layout == _layout) return 0;
        ...
        if  ( CGRectIsEmpty(bounds) || !_collectionViewFlags.doneFirstLayout) {
            ...
            CGPoint centerPoint = CGPoint::point(bounds.origin.x + bounds.size.width / 2.f,
                bounds.origin.y + bounds.size.height / 2.f);
            ...
            id layoutInterchangeData = [NSMutableDictionary dictionaryWithCapacity:[newlyVisibleLayoutAttrs count] + [previouslyVisibleItemsKeysSet count]];
            ...
            for (id attr in newlyVisibleLayoutAttrs) {
                ...
                if (prevAttr != nil && newAttr != nil) {
                    [layoutInterchangeData setObject:[NSDictionary dictionaryWithObjectsAndKeys:prevAttr, @"previousLayoutInfos", "newAttr", @"newLayoutInfos", NULL] forKey:newKey];
                }
            }
            ...
            for (UICollectionViewItemKey* key in previouslyVisibleItemsKeysSet) {
                ...
                if ( [key type] == UICollectionViewItemTypeDecorationView) {
                    ...
                    prevAttr = [[self collectionViewLayout] layoutAttributesForDecorationViewOfKind:[decorView reuseIdentifier] atIndexPath:[key indexPath]];
                    ...
                    prevAttr = [[self collectionViewLayout] layoutAttributesForSupplementaryViewOfKind:[[suuplView layoutAttributes] representedElementKind] atIndexPath:[key indexPath]];
                    newAttr = [layout layoutAttributesForSupplementaryViewOfKind:[[suuplView layoutAttributes] representedElementKind] atIndexPath:[key indexPath]];
                }

                id layoutInterchangeDataValue = [NSMutableDictionary dictionary];
                if (prevAttr) [layoutInterchangeDataValue setObject:prevAttr forKey:@"previousLayoutInfos"];
                if (newAttr) [layoutInterchangeDataValue setObject:newAttr forKey:@"newLayoutInfos"];
            ...
            for (UICollectionViewItemKey* key in [layoutInterchangeData keyEnumerator]) {
                if ( [key type] == UICollectionViewItemTypeCell ) {
                    ...
                    if (!cell) {
                        cell = [self createPreparedCellForItemAtIndexPath:[key indexPath] withLayoutAttributes:[[layoutInterchangeData objectForKey:key] objectForKey:@"previousLayoutInfos"]];
                        [_allVisibleViewsDict setObject:cell forKey:key];
                        [self addControlledSubview:cell];
                    }
                    else [cell applyLayoutAttributes:[[layoutInterchangeData objectForKey:key] objectForKey:@"previousLayoutInfos"]];
                }
                else if ( [key type] == UICollectionViewItemTypeSupplementaryView ) {
                    ...
                    if (!view) {
                        ...
                        view = [self createPreparedSupplementaryViewForElementOfKind:[attrs representedElementKind] atIndexPath:[attrs indexPath] withLayoutAttributes:attrs];
    ...
    /* annotate with type */ -(void) setCollectionViewLayout:(id)layout {
        [self setCollectionViewLayout:layout animated:NO];
    }

    /* annotate with type */ -(void) registerNib:(id)nib forCellWithReuseIdentifier:(id)identifier {
        [_cellNibDict setObject:nib forKey:identifier];
    }

    /* annotate with type */ -(void) registerNib:(id)nib forSupplementaryViewOfKind:(id)kind withReuseIdentifier:(id)identifier {
    ...
    /* annotate with type */ -(void) layoutSubviews {
    ...
    /* annotate with type */ -(id) invalidateLayout {
        [_layout invalidateLayout];
        [_collectionViewData invalidate]; // invalidate layout cache
        return self;
    }

    /* annotate with type */ -(id) collectionViewLayout {
        return _layout;
    }

    /* annotate with type */ -(id) collectionViewData {
        return _collectionViewData;
    }

    /* annotate with type */ -(void) reloadData {
    ...
    /* annotate with type */ -(id) indexPathsForSelectedItems {
    ...
    /* annotate with type */ -(id) updateVisibleCellsNow:(BOOL)now {
        CGRect bounds;
        bounds = [self bounds];
                ...
                if ( view ) {
                    ...
                    if ( [itemKey type] == UICollectionViewItemTypeCell) {
                        ...
                        if (_collectionViewFlags.delegateDidEndDisplayingSupplementaryView) {
                            [_collectionViewDelegate collectionView:self didEndDisplayingSupplementaryView:reusableView forElementOfKind:[itemKey identifier] atIndexPath:[itemKey indexPath]];
                        }
    ...
    /* annotate with type */ -(id) createPreparedCellForItemAtIndexPath:(NSIndexPath*)indexPath withLayoutAttributes:(id)layoutAttributes {
    ...
    /* annotate with type */ -(id) createPreparedSupplementaryViewForElementOfKind:(id)kind atIndexPath:(NSIndexPath*)indexPath withLayoutAttributes:(id)layoutAttributes {
    ...
    /* annotate with type */ -(id) dequeueReusableSupplementaryViewOfKind:(id)elementKind withReuseIdentifier:(id)identifier forIndexPath:(NSIndexPath*)indexPath {
    ...
    /* annotate with type */ -(id) dequeueReusableCellWithReuseIdentifier:(id)identifier forIndexPath:(NSIndexPath*)indexPath {
    ...
    /* annotate with type */ -(id) dequeueReusableOrCreateDecorationViewOfKind:(id)elementKind forIndexPath:(NSIndexPath*)indexPath {
    ...
    /* annotate with type */ -(id) addControlledSubview:(id)subview {
    ...
    /* annotate with type */ -(id) reuseCell:(id)cell {
        [self queueReusableView:cell inQueue:(id) _cellReuseQueues withIdentifier:[cell reuseIdentifier]];

        return 0;
    }

    /* annotate with type */ -(id) reuseSupplementaryView:(id)supplementaryView {
        id kindAndIdentifier = [NSString stringWithFormat:@"%@/%@", [[supplementaryView layoutAttributes] elementKind], [supplementaryView reuseIdentifier]];
        [self queueReusableView:supplementaryView inQueue:(id) _supplementaryViewReuseQueues withIdentifier:kindAndIdentifier];
        return 0;
    }

    /* annotate with type */ -(id) reuseDecorationView:(id)decorationView {
    ...
    /* annotate with type */ -(id) queueReusableView:(id)reusableView inQueue:(id)queue withIdentifier:(id)identifier {
    ...
    /* annotate with type */ -(void) touchesBegan:(id)touches withEvent:(id)event {
    ...
    /* annotate with type */ -(void) touchesMoved:(id)touches withEvent:(id)event {
        if (_touchingIndexPath) {
            CGPoint touchPoint = [[touches anyObject] locationInView:self];
            NSIndexPath* indexPath = [self indexPathForItemAtPoint:touchPoint];

            // moving out of bounds
            if ([_currentIndexPath isEqual:(id) _touchingIndexPath] &&
                ![indexPath isEqual:(id) _touchingIndexPath] &&
            ...
            }else if (![_currentIndexPath isEqual:(id) _touchingIndexPath] &&
                [indexPath isEqual:(id) _touchingIndexPath]) {
                [self highlightItemAtIndexPath:(id) _touchingIndexPath animated:YES scrollPosition:UICollectionViewScrollPositionNone notifyDelegate:YES];
    ...
    /* annotate with type */ -(id) cellTouchCancelled {
    ...
    /* annotate with type */ -(id) userSelectedItemAtIndexPath:(NSIndexPath*)indexPath {
    ...
    /* annotate with type */ -(id) selectItemAtIndexPath:(NSIndexPath*)indexPath animated:(BOOL)animated scrollPosition:(UICollectionViewScrollPosition)scrollPosition notifyDelegate:(BOOL)notifyDelegate {
        if ( [self allowsMultipleSelection] && [_indexPathsForSelectedItems containsObject:indexPath]) {
    ...
    /* annotate with type */ -(void) touchesCancelled:(id)touches withEvent:(id)event {
    ...
    /* annotate with type */ -(id) indexPathForItemAtPoint:(CGPoint)point {
    ...
    /* annotate with type */ -(id) cellForItemAtIndexPath:(id)indexPath {
        id ret = nil;
    ...
    /* annotate with type */ -(void) deselectItemAtIndexPath:(id)indexPath animated:(BOOL)animated {
        [self deselectItemAtIndexPath:indexPath animated:animated notifyDelegate:NO];
    }

    /* annotate with type */ -(void) performBatchUpdates:(void(^)())updates completion:(void(^)(BOOL))completion {
        [self setupCellAnimations];

        if (updates) EbrCallBlock(updates, "d", updates);
        if (completion) _updateCompletionHandler.attach([completion copy]);

        [self endItemAnimations];
    }

    /* annotate with type */ -(id) setupCellAnimations {
        [self updateVisibleCellsNow:YES];
        [self suspendReloads];
    ...
    /* annotate with type */ -(id) suspendReloads {
        _reloadingSuspendedCount++;
        return 0;
    }

    /* annotate with type */ -(id) resumeReloads {
        if (0 < _reloadingSuspendedCount) _reloadingSuspendedCount--;
        return self;
    }

    /* annotate with type */ -(void) reloadSections:(id)sections {
        [self updateSections:sections updateAction:UICollectionUpdateActionReload];
    }

    /* annotate with type */ -(id) updateSections:(id)sections updateAction:(UICollectionUpdateAction)updateAction {
        ...
        while(idx != NSNotFound)
        {
            ...
            id updateItem = [[UICollectionViewUpdateItem alloc] initWithAction:updateAction forIndexPath:[NSIndexPath indexPathForItem:NSNotFound inSection:section]];
    ...
    /* annotate with type */ -(id) arrayForUpdateAction:(UICollectionUpdateAction)updateAction {
        id updateActions = nil;

        switch (updateAction) {
            case UICollectionUpdateActionInsert:
                if (!_insertItems) _insertItems.attach([NSMutableArray new]);
                updateActions = _insertItems;
                break;
            case UICollectionUpdateActionDelete:
                if (!_deleteItems) _deleteItems.attach([NSMutableArray new]);
                updateActions = _deleteItems;
                break;
            case UICollectionUpdateActionMove:
                if (!_moveItems) _moveItems.attach([NSMutableArray new]);
                updateActions = _moveItems;
                break;
            case UICollectionUpdateActionReload:
                if (!_reloadItems) _reloadItems.attach([NSMutableArray new]);
    ...
    /* annotate with type */ -(id) currentUpdate {
        return _update;
    }

    /* annotate with type */ -(id) visibleViewsDict {
    ...
    /* annotate with type */ -(id) prepareLayoutForUpdates {
    ...
    /* annotate with type */ -(id) updateWithItems:(id)items {
        ...
        for (UICollectionViewUpdateItem* updateItem in items) {
            ...
            if ( [updateItem isSectionOperation] && [updateItem updateAction] == UICollectionUpdateActionDelete) {
                NSInteger numberOfBeforeSection = [[_update objectForKey:@"oldModel"] numberOfItemsInSection:[[updateItem indexPathBeforeUpdate] section]];
        ...
        for (UICollectionViewItemKey* key in [_allVisibleViewsDict keyEnumerator]) {
            ...
            if ( [key type] == UICollectionViewItemTypeCell) {
                ...
                id newIndexPath = newGlobalIndex == NSNotFound ? nil : [[_update objectForKey:@"newModel"] indexPathForItemAtGlobalIndex:newGlobalIndex];
                id oldIndexPath = oldGlobalIndex == NSNotFound ? nil : [[_update objectForKey:@"oldModel"] indexPathForItemAtGlobalIndex:oldGlobalIndex];
                ...
                startAttrs = [view layoutAttributes];
                finalAttrs = [_layout layoutAttributesForDecorationViewOfKind:[[view layoutAttributes] representedElementKind] atIndexPath:[key indexPath]];
    ...
    /* annotate with type */ -(id) _doUpdateCompletion:(BOOL)finished :(id)viewsToRemove {
    ...
    /* annotate with type */ -(id) _doUpdateAnimations:(id)animations {
        _collectionViewFlags.updatingLayout = YES;
    ...
    /* annotate with type */ -(id) endItemAnimations {
        _updateCount++;
        ...
        id removeUpdateItems = [[self arrayForUpdateAction:UICollectionUpdateActionDelete] sortedArrayUsingSelector:@selector(inverseCompareIndexPaths:)];

        id insertUpdateItems = [[self arrayForUpdateAction:UICollectionUpdateActionInsert] sortedArrayUsingSelector:@selector(compareIndexPaths:)];
        ...
        for (id updateItem in sortedMutableReloadItems) {
            ...
            if ( [[updateItem indexPathBeforeUpdate] item] != NSNotFound) {
                assert([[updateItem indexPathBeforeUpdate] item] < [oldCollectionViewData numberOfItemsInSection:[[updateItem indexPathBeforeUpdate] section]]);
            }

            [someMutableArr2 addObject:[[UICollectionViewUpdateItem alloc] initWithAction:UICollectionUpdateActionDelete forIndexPath:[updateItem indexPathBeforeUpdate]]];
            [someMutableArr3 addObject:[[UICollectionViewUpdateItem alloc] initWithAction:UICollectionUpdateActionInsert forIndexPath:[updateItem indexPathAfterUpdate]]];
        }
        ...
        for (id deleteItem in sortedDeletedMutableItems) {
            if ( [deleteItem isSectionOperation]) {
                ...
                assert([[deleteItem indexPathBeforeUpdate] section] < [oldCollectionViewData numberOfSections]);
                assert([[deleteItem indexPathBeforeUpdate] item] < [oldCollectionViewData numberOfItemsInSection:[[deleteItem indexPathBeforeUpdate] section]]);
            ...
            if (sectionOperation) {
                ...
                for (id moveItem in sortedMutableMoveItems) {
                    if ( [[moveItem indexPathAfterUpdate] isEqual:indexPath]) {
                        if ( [moveItem isSectionOperation])
                            assert(0); //NO, @"attempt to perform an insert and a move to the same section (%ld)", (long)indexPath.section);
                    }
                }
        ...
        for (id sortedItem in sortedMutableMoveItems) {
            if ( [sortedItem isSectionOperation]) {
                ...
                */
                assert([[sortedItem indexPathBeforeUpdate] item] < [oldCollectionViewData numberOfItemsInSection:[[sortedItem indexPathBeforeUpdate] section]]);
                /*
                ...
                */
                ...
                (long)[_collectionViewData numberOfSections]);
                */
                assert([[sortedItem indexPathAfterUpdate] item] < [_collectionViewData numberOfItemsInSection:[[sortedItem indexPathAfterUpdate] section]]);
                /*
            ...
            switch ( [updateItem updateAction] ) {
                case UICollectionUpdateActionDelete: {
                    if ( [updateItem isSectionOperation] ) {
                        ...
                        [[newModel objectAtIndex:[[updateItem indexPathBeforeUpdate] section]] removeObjectAtIndex:[[updateItem indexPathBeforeUpdate] item]];
                    }
                }
                    break;
                case UICollectionUpdateActionInsert: {
                    if ([updateItem isSectionOperation]) {
                        ...
                        [[newModel objectAtIndex:[[updateItem indexPathAfterUpdate] section]] insertObject:[NSNumber numberWithInt:NSNotFound] atIndex:[[updateItem indexPathAfterUpdate] item]];
                    }
                }
                    break;
                ...
                case UICollectionUpdateActionMove: {
                    if ([updateItem isSectionOperation]) {
                        ...
                        id object = [NSNumber numberWithInt:[oldCollectionViewData globalIndexForItemAtIndexPath:[updateItem indexPathBeforeUpdate]]];
                        [[newModel objectAtIndex:[[updateItem indexPathBeforeUpdate] section]] removeObject:object];
                        [[newModel objectAtIndex:[[updateItem indexPathAfterUpdate] section]] insertObject:object atIndex:[[updateItem indexPathAfterUpdate] item]];
                    }
                }
                    break;
                default: break;
