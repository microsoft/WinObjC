...
@implementation UITableViewCell : UIView
    ...
    -(void) updateBackgroundView:(BOOL)highlight {
        ...
        if ( _currentlyHighlighted && !wasHighlighted ) {
            ...
            if ( _selectionStyle != UITableViewCellSelectionStyleNone ) [self setHighlighted:highlight forViews:[_contentView subviews]];
    ...
    -(void) touchesBegan:(NSSet*)touches withEvent:(UIEvent*)event {
        if ( [priv->superview respondsToSelector:@selector(_cellSelectedDown:)] ) [priv->superview _cellSelectedDown:self];
    }
    ...
    -(void) touchesCancelled:(NSSet*)touches withEvent:(UIEvent*)event {
        if ( [priv->superview respondsToSelector:@selector(_cellSelectedCancelled:)] ) [priv->superview _cellSelectedCancelled:self];
    }

    -(void) touchesEnded:(NSSet*)touches withEvent:(UIEvent*)event {
        if ( [priv->superview respondsToSelector:@selector(_cellSelectedUp:)] ) [priv->superview _cellSelectedUp:self];
    }

    -(void) __accessoryTapped {
        if ( [priv->superview respondsToSelector:@selector(_cellAccessoryTapped:)] ) [priv->superview _cellAccessoryTapped:self];
    }
    ...
    static void setInternalAccessoryColor(UITableViewCell* self)
    {
        if ( self->_internalAccessoryView != nil ) {
            ...
            [[self->_internalAccessoryView layer] _setContentColor: [UIColor colorWithRed: contentColor.r green: contentColor.g blue: contentColor.b alpha: contentColor.a]];
        }
    }
        ...
        if ( type == UITableViewCellAccessoryDetailDisclosureButton ) {
            if ( self->_internalAccessoryButton == nil ) {
                if ( self->_internalAccessoryButton == nil ) {
                    ...
                    [self->_internalAccessoryButton addTarget:self action:@selector(__accessoryTapped) forControlEvents:UIControlEventTouchUpInside];
                }
    ...
    -(void) layoutSubviews {
        ...
        if ( _style == 1 ) {
            ...
            if ( textLabelSize.size.width < 0 ) textLabelSize.size.width = 0;
    ...
    -(void) __didLayout {
        if ( _deferredIndexPath != nil ) {
            if ( _indexPath == nil ) _indexPath = _deferredIndexPath;
            _deferredIndexPath = nil;
        }
        setupGroupView(self);
    }
