...
@implementation UIImageView : UIView {
    ...
    -(void) setAnimationDuration:(double)duration {
        ...
        if ( imgPriv->_timer != nil && imgPriv->_isAnimating && changed ) {
            ...
            imgPriv->_timer = [NSTimer scheduledTimerWithTimeInterval:duration target:self selector:@selector(_showNextImage) userInfo:0 repeats:TRUE];
        }
    }
    ...
    -(void) startAnimating {
        ...
        if ( imgPriv->_isAnimating ) return;
