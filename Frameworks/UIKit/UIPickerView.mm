...
@implementation UIPickerSubView : UIScrollView
    ...
    /* annotate with type */ -(id) reloadComponent {
    ...
    static void showVisibleCells(UIPickerSubView* self)
    {
        ...
        for ( int j = 0; j < self->_numRows; j ++ ) {
            ...
            if ( curRow->_yPos + self->_rowHeight  < scrollPoint.y || 
                ...
                if ( curRow->_rowCell == nil || (curRow->_rowCell != nil && curRow->_invalid) ) {
                    ...
                    if ( [self->_dataSource respondsToSelector:@selector(pickerView:titleForRow:forComponent:)] || 
                        ...
                        if ( [self->_dataSource respondsToSelector:@selector(pickerView:titleForRow:forComponent:withColor:)] ) {
                            rowString = [[self->_dataSource pickerView:self->_parent titleForRow:j forComponent:self->_componentNum withColor:&rowColor] retain];
                        } else {
                        ...
                        curRow->_rowCell = [[self->_dataSource pickerView:self->_parent viewForRow:j forComponent:self->_componentNum reusingView:nil] retain];
                        [curRow->_rowCell setTag:j];
                    }
    ...
    static int findNearestRow(UIPickerSubView* self, CGPoint offset)
    {
        ...
        if ( row >= self->_numRows ) row = self->_numRows - 1;
        if ( row < 0 ) row = 0;
    ...
    /* annotate with type */ -(id) _selectRow:(int)row animated:(BOOL)animated {
        _selectedRow = row;
    ...
    /* annotate with type */ -(id) invalidateComponents {
        for ( int j = 0; j < _numRows; j ++ ) {
    ...
    /* annotate with type */ -(id) scrollViewWillBeginDragging:(id)scrollView {
        _targetSelectedRow = -1;
    ...
    /* annotate with type */ -(id) scrollViewDidEndDragging:(id)scrollView willDecelerate:(BOOL)decelerate {
        if ( decelerate )
            return self;
    ...
    /* annotate with type */ -(id) scrollViewWillEndDragging:(id)scrollView withVelocity:(CGPoint)velocity targetContentOffset:(CGPoint *)destination {
    ...
    /* annotate with type */ -(id) _cellSelectedUp:(id)cell {
        int selIdx = [cell tag];
        CGPoint dest = { 0, getRowContentPos(self, selIdx) };
...
@implementation UIPickerView : UIView {
    ...
    static void setupImages(UIPickerView* self)
    {
        ...
        self->_background = [[[UIImageView alloc] initWithImage:[[UIImage imageNamed:@"/img/wheel_bg@2x.png"] stretchableImageWithLeftCapWidth:4 topCapHeight:0]] autorelease];
        ...
        self->_selectionBar = [[[UIImageView alloc] initWithImage:[[UIImage imageNamed:@"/img/picker-selection@2x.png"] stretchableImageWithLeftCapWidth:8 topCapHeight:0]] autorelease];
    ...
    /* annotate with type */ -(id) setShowsSelectionIndicator:(DWORD)show {
        return self;
    }
    ...
    /* annotate with type */ -(id) setDataSource:(id)source {
        _dataSource = source;
        _needsReload = TRUE;
        [self _setShouldLayout];
    ...
    -(id) /* use typed version */ reloadAllComponents {
        ...
        if ( dataSource == nil ) dataSource = _delegate;
    ...
    /* annotate with type */ -(id) reloadComponent:(int)component {
        [self reloadAllComponents];

        return 0;
    }

    /* annotate with type */ -(id) _subCellSelected:(int)row fromPicker:(id)fromPicker {
    ...
    /* annotate with type */ -(id) selectRow:(int)row inComponent:(int)component animated:(BOOL)animated {
        //assert( component == 0 );
        _selectedRowInComponents[component] = row;

        if ( component >= _numSections ) return self;
    ...
    /* annotate with type */ -(id) _setAlignment:(UITextAlignment)alignment forComponent:(int)component {
    ...
    -(CGSize) rowSizeForComponent:(int)component {
        ...
        if ( component >= _numSections ) return ret;
    ...
    /* annotate with type */ -(id) setMaximumDate:(id)date {
        return self;
    }
