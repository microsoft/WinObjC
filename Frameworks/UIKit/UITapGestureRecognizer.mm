...
@implementation UITapGestureRecognizer  {
    ...
    /* annotate with type */ -(id) initWithTarget:(id)target action:(SEL)selector {
    ...
    /* annotate with type */ -(void) setNumberOfTapsRequired:(unsigned)numberOfTaps {
    ...
    /* annotate with type */ -(void) setNumberOfTouchesRequired:(unsigned)numberOfTouches {
    ...
    /* annotate with type */ -(id) touchesBegan:(id)touches withEvent:(id)event {
        ...
        if ( _numberOfTapsRequired == 1 ) {
            ...
            if ( _recognizeTimer != nil ) [_recognizeTimer invalidate];
            if ( shouldDelay ) {
                _recognizeTimer = [NSTimer scheduledTimerWithTimeInterval:0.35 target:self selector:@selector(_fireRecognized) userInfo:nil repeats:FALSE];
                if ( _pendingTaps == nil ) {
    ...
    /* annotate with type */ -(id) touchesMoved:(id)touches withEvent:(id)event {
        id curTouch = [touches anyObject];
        CGPoint curPos;
        curPos = [curTouch locationInView:nil];
    ...
    /* annotate with type */ -(id) touchesEnded:(id)touches withEvent:(id)event {
        id curTouch = [touches anyObject];
        CGPoint curPos;
        curPos = [curTouch locationInView:nil];
        savedTouch *save = findTouch(self, curTouch);
    ...
    /* annotate with type */ -(id) _fireRecognized {
        if ( _wasRecognized ) {
            ...
            [_pendingTaps removeObject:self];
            if ( fireGesture == nil ) fireGesture = self;
