...
@implementation UIBarButtonItem : UIBarItem {
    ...
    {
        if ( self->_systemItem != UIBarButtonSystemItemFlexibleSpace && self->_systemItem != UIBarButtonSystemItemFixedSpace && self->_customView == nil ) {
    ...
    -(instancetype) initWithBarButtonSystemItem:(UIBarButtonSystemItem)sysItem target:(id)target action:(SEL)targetSel {
        ...
        switch ( sysItem ) {
            case UIBarButtonSystemItemDone:
                _title = @"Done";
                { CGSize size; size = [_title sizeWithFont:(id) _font]; _width = size.width + 15.0f; }
                break;
    ...
    -(float) _margin {
        if ( _systemItem == UIBarButtonSystemItemFlexibleSpace || _systemItem == UIBarButtonSystemItemFixedSpace ) return 0.0f;
