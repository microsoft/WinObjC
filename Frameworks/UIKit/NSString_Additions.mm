...
@implementation NSString(UIKitAdditions)
    static void drawCharsAtPoint(UIFont* font, CGContextRef context, WORD *str, DWORD strLength, CGRect rct, float x, float y, UITextAlignment alignment, CGSize *sizeOut)
    {
    ...
    static void drawString(UIFont* font, CGContextRef context, WORD *str, DWORD strLength, CGRect rct, UILineBreakMode lineBreakMode, UITextAlignment alignment, CGSize *sizeOut, CGPoint *lastCharPos = NULL)
    {
        if ( font == nil ) {
        ...
        switch ( lineBreakMode ) {
            case UILineBreakModeWordWrap: {
                ...
                while ( CGFontWrap(font, size, &state, rct.size.width, &fitSize) ) {
                    ...
                    if ( extent.width > sizeOut->width ) sizeOut->width = extent.width;
                    if ( y + extent.height - rct.origin.y > sizeOut->height ) sizeOut->height = y + extent.height - rct.origin.y;
                ...
                while ( CGFontWrap(font, size, &state, rct.size.width, &fitSize) ) {
                    if ( y + (fitSize.height * 2) - rct.origin.y > rct.size.height ) {
                        ...
                        if ( totalSize.width <= rct.size.width && numTotal == strLength ) {
                            ...
                            if ( CGFontFitChars(font, size, ellipseText, 3, rct.size.width, &ellipseSize) < 3 ) break;
    ...
    -(CGSize) drawAtPoint:(CGPoint)pt  forWidth:(float)forWidth  withFont:(UIFont*)font  minFontSize:(float)minFontSize  actualFontSize:(float *)actualFontSize  lineBreakMode:(DWORD)lineBreak  baselineAdjustment:(DWORD)baseline {
        CGSize fontExtent;
        WORD *str = (WORD *) [self rawCharacters];
        ...
        drawString(font, UIGraphicsGetCurrentContext(), str, [self length], rct, UILineBreakModeClip, UITextAlignmentLeft, &fontExtent);
        if ( actualFontSize) *actualFontSize = 10.0f;

        return fontExtent;
    }

    -(CGSize) drawAtPoint:(CGPoint)pt  forWidth:(float)forWidth  withFont:(UIFont*)font  fontSize:(float)fontSize  lineBreakMode:(DWORD)lineBreak  baselineAdjustment:(DWORD)baseline {
        CGSize fontExtent;
        WORD *str = (WORD *) [self rawCharacters];
    ...
    -(CGSize) sizeWithFont:(UIFont*)font  minFontSize:(float)minFontSize  actualFontSize:(float *)actualFontSize  forWidth:(float)forWidth  lineBreakMode:(UILineBreakMode)lineBreak {
        CGSize ret;
        WORD *str = (WORD *) [self rawCharacters];
    ...
    -(CGSize) sizeWithFont:(UIFont*)font {
        if ( font == nil ) font = [objc_getClass("UIFont") defaultFont];
