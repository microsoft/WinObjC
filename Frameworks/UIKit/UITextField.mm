...
@implementation UITextField {
    ...
    -(instancetype) initWithCoder:(NSCoder *)coder {
        ...
        if ( _text == nil ) _text = @"";
        if ( __textColor == nil ) __textColor = [UIColor blackColor];
    ...
    -(void) drawRect:(CGRect)rect {
        ...
        if ( _text == nil || [_text length] == 0 ) {
            ...
            if ( _secureTextMode ) {
                ...
                for ( unsigned i = 0; i < [text length] - _showLastCharLen; i ++ ) chars[i] ='*';
        ...
        if ( _borderStyle != UITextBorderStyleNone ) {
            if ( [[self layer] borderWidth] == 0.0f ||
                ...
                switch ( _borderStyle ) {
                    case UITextBorderStyleLine: {
                        ...
                        if ( [self isFirstResponder] ) {
                            CGContextSetStrokeColorWithColor(curContext, (CGColorRef) (_tintColor ? [_tintColor CGColor] : [[UIColor redColor] CGColor]));
                        } else {
                    ...
                    case UITextBorderStyleBezel: {
                        rect = [self bounds];
                        id image = [[UIImage imageNamed:@"/img/TextFieldBezel@2x.png"] resizableImageWithCapInsets: UIEdgeInsetsMake(8, 8, 8, 8)];
                        rect = [self bounds];
                        [image drawInRect:rect];
                    }
                    break;
        ...
        if ( !_isPlaceholder ) rect.origin.x += size.width;
        rect.size.width = 2;
    ...
    -(CGSize) sizeThatFits:(CGSize)curSize {
        ...
        if ( _placeholder != nil ) {
            placeholderSize = [_placeholder sizeWithFont:_font constrainedToSize:CGSizeMake(curSize.width, curSize.height) lineBreakMode:UILineBreakModeClip];
        } 
