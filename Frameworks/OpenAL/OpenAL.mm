...
public:
    ...
    bool FillBuffer(ALBufferPlayback *bufInfo)
    {
        ...
        int framesToCopy = sourceFramesAvailable < (bufInfo->_frameBufMax - bufInfo->_frameBufLen) ? sourceFramesAvailable : (bufInfo->_frameBufMax - bufInfo->_frameBufLen);
...
public:
    ...
    {
        if ( _state != AL_PLAYING ) return false;
...
ALCcontext_struct::ALCcontext_struct(ALCdevice *parent) :
    _parent(parent)
{
    _curBufId = 1;
}
...
{
    if ( _sources.find(source) == _sources.end() ) return;
...
void ALCdevice_struct::AudioPlaybackThread()
{
    ...
    void (^activateAudioOnMainThread)() = ^void() {
        ...
        WindowsCreateString(RuntimeClass_Windows_Media_Devices_MediaDevice, wcslen(RuntimeClass_Windows_Media_Devices_MediaDevice), &mediaDeviceClass);
        ...
        activateResult = ActivateAudioInterfaceAsync(deviceName, (REFIID) __uuidof(IAudioClient2), NULL, (IActivateAudioInterfaceCompletionHandler *) &openCallback, &complete);
        free(deviceName);
    ...
    while ( !_terminateThread ) {
        ...
        if (renderClient) {
            ...
            framesToFill = 2048;
            if (!sampleBacking) sampleBacking = (BYTE*)realloc(sampleBacking, framesToFill * outputFmt.nBlockAlign);
        ...
        if ( framesToFill > 0 ) {
            ...
            if (renderClient) renderClient->ReleaseBuffer(framesToFill, 0);
