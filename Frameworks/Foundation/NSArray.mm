...
@implementation NSArray : NSObject
    +(NSArray*) arrayWithObjects:(NSObject*)first , ...{
        va_list pReader; va_start(pReader, first);
        NSArray* ret = [self new];
        if ( first == nil ) return [ret autorelease];
    ...
    -(NSArray*) initWithArray:(NSArray*)arrayToCopy {
        if ( arrayToCopy != nil && (object_getClass(arrayToCopy) == [NSArrayConcrete class] || object_getClass(arrayToCopy) == [NSMutableArrayConcrete class]) ) {            
            int objCount = CFArrayGetCount((CFArrayRef) arrayToCopy);
            id *objs = NULL;
            
            if ( objCount > 0 ) objs = (id *) _CFArrayGetPtr((CFArrayRef) arrayToCopy);
    ...
    -(NSString*) componentsJoinedByString:(NSString*)str {
        ...
        while ( nextObject != nil ) {
            if ( addSeparator ) [ret appendString:str];
            addSeparator = true;
    ...
    -(NSUInteger) indexOfObject:(NSObject*)obj inSortedRange:(NSRange)range options:(NSBinarySearchingOptions)options usingComparator:(NSComparator)comparator {
        if ( range.length == 0 ) {
