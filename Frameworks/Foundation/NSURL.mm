...
struct EbrURL
{
    ...
    ~EbrURL()
    {
        ...
        if ( _parameters ) free(_parameters);
        if ( _path ) free(_path);
    }
    ...
    void AppendPath(char *pPath)
    {
        ...
        if ( _path ) newLen += strlen(_path);
        if ( pPath ) newLen += strlen(pPath);

        //  Strip out parameter
        char *newPath = (char *) malloc(newLen);
        strcpy(newPath, "");
        if ( _path ) strcpy(newPath, _path);
        strcat(newPath, "/");
...
@implementation NSURL : NSObject
    ...
    -(instancetype) initFileURLWithPath:(NSString*)path {
        ...
        if ( [path isAbsolutePath] == FALSE ) 
        {
            ...
            //  Add trailing /
            if ( strlen(szBasePath) > 0 && szBasePath[strlen(szBasePath) - 1] != '/' ) strcat(szBasePath, "/");
        }
        ...
        if ( EbrAccess(szPath, 0) == -1 || EbrIsDir(szPath) ) isDirectory = TRUE;
        free(szPath);
    ...
    -(instancetype) initFileURLWithPath:(NSString*)path isDirectory:(BOOL)isDirectory {
        ...
        if ( isDirectory ) {
            //  Add trailing /
            if ( strlen(szPath) > 0 && szPath[strlen(szPath) - 1] != '/' ) strcat(szPath, "/");
        }
    ...
    /* annotate with type */ -(id) initWithString:(id)string relativeToURL:(id)parent isDirectory:(DWORD)isDirectory {
        if ( string == nil ) return nil;
    ...
    -(BOOL) isEqual:(id)other {
        ...
        if ( otherURL->_fullUri->_scheme && _fullUri->_scheme ) {
            if( strcmp(otherURL->_fullUri->_scheme, _fullUri->_scheme) != 0 ) return NO;
        } else if ( otherURL->_fullUri->_scheme != _fullUri->_scheme ) return NO;

        if ( otherURL->_fullUri->_server && _fullUri->_server ) {
            if( strcmp(otherURL->_fullUri->_server, _fullUri->_server) != 0 ) return NO;
        } else if ( otherURL->_fullUri->_server != _fullUri->_server ) return NO;
   
        if ( otherURL->_fullUri->_path && _fullUri->_path ) {
            if( strcmp(otherURL->_fullUri->_path, _fullUri->_path) != 0 ) return NO;
        } else if ( otherURL->_fullUri->_path != _fullUri->_path ) return NO;
   
        if ( otherURL->_fullUri->_parameters && _fullUri->_parameters ) {
            if( strcmp(otherURL->_fullUri->_parameters, _fullUri->_parameters) != 0 ) return NO;
        } else if ( otherURL->_fullUri->_parameters != _fullUri->_parameters ) return NO;

        if ( otherURL->_fullUri->_query && _fullUri->_query ) {
            if( strcmp(otherURL->_fullUri->_query, _fullUri->_query) != 0 ) return NO;
        } else if ( otherURL->_fullUri->_query != _fullUri->_query ) return NO;

        if ( otherURL->_fullUri->_fragment && _fullUri->_fragment ) {
            if( strcmp(otherURL->_fullUri->_fragment, _fullUri->_fragment) != 0 ) return NO;
        } else if ( otherURL->_fullUri->_fragment != _fullUri->_fragment ) return NO;

        if ( otherURL->_fullUri->_port != _fullUri->_port ) return NO;
    ...
    -(NSString*) parameterString {
        if ( !_fullUri->_parameters ) return nil;
    ...
    -(NSString*) scheme {
        if ( !_fullUri ) return nil;
        if ( !_fullUri->_scheme ) return nil;
    ...
    -(NSString*) fragment {
        if ( !_fullUri->_fragment ) return nil;
    ...
    /* annotate with type */ -(id) path {
        if ( !_fullUri ) return nil;
        if ( !_fullUri->_path ) return nil;
    ...
    /* annotate with type */ -(id) pathComponents {
        return [[self path] pathComponents];
    }

    /* annotate with type */ -(id) port {
        if ( _fullUri->_port == -1 ) return nil;

        return [NSNumber numberWithInt:_fullUri->_port];
    }

    /* annotate with type */ -(id) query {
        if ( !_fullUri->_query ) return nil;
    ...
    /* annotate with type */ -(id) relativePath {
        if ( !_uri->_path ) return nil;
    ...
    /* annotate with type */ -(id) description {
        return [self absoluteString];
    }

    /* annotate with type */ -(id) absoluteURL {
        if(_baseURL==nil)
            return self;
    ...
    /* annotate with type */ -(id) standardizedURL {
        EbrDebugLog("standardizedURL needs attention\n");
        return [self copy];
    }

    /* annotate with type */ -(id) lastPathComponent {
        return [[self path] lastPathComponent];
    }

    /* annotate with type */ -(id) pathExtension {
        return [[self path] pathExtension];
    }

    /* annotate with type */ -(id) baseURL {
        return _baseURL;
    }
