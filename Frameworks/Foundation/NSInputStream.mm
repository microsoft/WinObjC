...
@implementation NSInputStream : NSStream
    /* annotate with type */ +(id) inputStreamWithFileAtPath:(id)file {
    ...
    /* annotate with type */ +(id) inputStreamWithData:(id)data {
        id ret = [self alloc];

        return [[ret initWithData:data] autorelease];
    }

    /* annotate with type */ -(id) initWithData:(id)data {
        _data = data;

        return self;
    }

    /* annotate with type */ -(id) initWithFileAtPath:(id)file {
        if ( file == nil ) {
    ...
    -(int) read:(char *)buf maxLength:(unsigned)maxLength {
        if ( _data == nil ) {
            ...
            assert( toRead >= 0 );
            if ( toRead > (int)maxLength ) toRead = (int)maxLength;
