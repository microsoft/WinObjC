...
@implementation NSKeyedArchiver : NSCoder
    ...
    */
    -(NSDictionary*) _encodeObject:(id)anObject conditional:(BOOL)conditional {
        ...
        if ( anObject != nil ) {
            ...
            if ( _priv->repMap.get(anObject, tempObj) ) {
                ...
                if (_delegate != nil)
                {
                    if (anObject != nil)  anObject = [_delegate archiver:self willEncodeObject:anObject];
                    if (original != anObject) [_delegate archiver:self willReplaceObject:original withObject:anObject];
                }
    ...
    -(NSString*) classNameForClass:(id)aClass {
        id* nameTmp;
        if ( _priv->clsMap.get(aClass, nameTmp) ) return *nameTmp;
        return nil;
    }
