...
static unsigned positionOfRangeLessThanOrEqualToLocation(NSRange *ranges, unsigned length, unsigned location) {
    ...
    while (--i >= 0) {
        if (ranges[i].location <= location) return i;
    }
...
static void raInsertItem(NSIndexSet* set, NSRange r, NSUInteger idx)
{
    ...
    //  Shift ranges up
    memmove(&set->_ranges[idx + 1], &set->_ranges[idx],
        sizeof(NSRange) * (set->_length - idx));
    set->_ranges[idx] = r;
    set->_length ++;
}
...
static void raRemoveItemAtIndex(NSIndexSet* set, NSUInteger idx)
{
    ...
    //  Shift ranges down
    memmove(&set->_ranges[idx], &set->_ranges[idx + 1],
        sizeof(NSRange) * (set->_length - (idx + 1)));
    set->_length --;
}
...
@implementation NSIndexSet : NSObject
    /* annotate with type */ +(id) indexSetWithIndexesInRange:(NSRange)range {
    ...
    /* annotate with type */ +(id) indexSet {
        id ret = [NSIndexSet new];

        return [ret autorelease];
    }

    /* annotate with type */ +(id) indexSetWithIndex:(NSUInteger)index {
        NSRange range;
    ...
    /* annotate with type */ -(id) initWithIndex:(unsigned)index {
        NSRange range;
    ...
    /* annotate with type */ -(id) initWithIndexSet:(NSIndexSet*)other {
    ...
    -(unsigned) countOfIndexesInRange:(NSRange)range {
        ...
        for ( unsigned i = 0; i < raCount(self); i++ )
        {
            ...
            if ( cur.location > range.location + range.length ) break;
            if ( cur.location + cur.length < range.location ) continue;
    ...
    /* annotate with type */ -(id) enumerateIndexesUsingBlock:(id)block {
        BOOL stop = FALSE;
    ...
    -(BOOL) intersectsIndexesInRange:(NSRange)range {
        ...
        if ( first == NSNotFound ) return NO;
   
        return (_ranges[first].location < NSMaxRange(range)) ? YES : NO;    
   }

    /* annotate with type */ -(id) mutableCopyWithZone:(NSZone*)zone {
    ...
    -(BOOL) isEqualToIndexSet:(NSIndexSet*)otherSet {
        if ( ![otherSet isKindOfClass:[NSIndexSet class]] ) return FALSE;
        ...
        if (count > 0)
        {
            ...
            for (i = 0; i < count; i++)
            {
                ...
                if (rself.location != rother.location ||
                rself.length != rother.length )
                {
                    return NO;
                }
            }
...
@implementation NSMutableIndexSet : NSIndexSet
    ...
    /* annotate with type */ -(id) removeIndexesInRange:(NSRange)range {
        ...
        while(range.length>0 && pos<_length) {
            if(_ranges[pos].location>=NSMaxRange(range)) break;
            ...
            if ( NSMaxRange(_ranges[pos]) == NSMaxRange(range)) {   
                if ( _ranges[pos].location==range.location ) {
                    ...
                    _length--;
                } else _ranges[pos].length=range.location-_ranges[pos].location;    
                break;
            }
    ...
    /* annotate with type */ -(id) shiftIndexesStartingAtIndex:(NSUInteger)index by:(NSInteger)delta {
        if ( delta < 0 ) {
            delta = -delta;
    ...
    /* annotate with type */ -(id) removeAllIndexes {
        _length=0;
        return self;
    }
    ...
    /* annotate with type */ -(id) _removeFromArray:(id)array {
        int i;
