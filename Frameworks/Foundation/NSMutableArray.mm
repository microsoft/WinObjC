...
@implementation NSMutableArray : NSArray
    ...
    static void shortsort(NSMutableArray* self, uint32_t lo, uint32_t hi, uint32_t compFunc, uint32_t context)
    {
        ...
        while (hi > lo) 
        {
            ...
            for (p = lo + 1; p <= (int) hi; p += 1) {
                if (((signed int) EbrCall(compFunc, "ddd", [self objectAtIndex:p], [self objectAtIndex:max], context)) > 0) max = p;
            }
    ...
    static void shortsort(NSMutableArray* self, uint32_t lo, uint32_t hi, SEL selector)
    {
        ...
        while (hi > lo) 
        {
            max = lo;
            for (p = lo + 1; p <= hi; p += 1) {
                if (selComp(self, p, max, selector) > 0) max = p;
            }
    ...
    -(void) sortUsingFunction:(uint32_t)compFunc context:(uint32_t)context range:(NSRange)range {
        ...
        } else 
        {
            ...
            for (;;) 
            {
                do { 
                    loguy += 1; 
                } while (loguy <= hi && ((signed int) EbrCall(compFunc, "ddd", [self objectAtIndex:loguy], [self objectAtIndex:lo], context)) <= 0);

                do { 
                    higuy -= 1; 
                } while (higuy > lo && ((signed int) EbrCall(compFunc, "ddd", [self objectAtIndex:higuy], [self objectAtIndex:lo], context)) >= 0);
