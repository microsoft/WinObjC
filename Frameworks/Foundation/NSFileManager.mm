...
DEFN_STR(NSFileTypeDirectory) DEFN_STR(NSFileTypeRegular) DEFN_STR(NSFileType) DEFN_STR(NSFileSize) DEFN_STR(NSFileModificationDate)
...
@implementation NSFileManager : NSObject
    ...
    -(BOOL) copyItemAtPath:(id)srcPath toPath:(id)destPath error:(id *)error {
        ...
        if ( EbrAccess(dest, 0) == 0 ) {
            if ( error ) *error = [NSError errorWithDomain:@"Would overwrite destination" code:100 userInfo:nil];
    ...
    /* annotate with type */ -(id) URLForDirectory:(DWORD)directory inDomain:(DWORD)domains appropriateForURL:(id)forURL create:(BOOL)create error:(id *)error {
        assert(forURL == nil);
        id paths = NSSearchPathForDirectoriesInDomains(directory, domains, TRUE);
    ...
    -(BOOL) contentsEqualAtPath:(id)pathObj1 andPath:(id)pathObj2 {
        ...
        if(dir != EbrIsDir(path2)) return false;
        ...
        if ( dir ) {
            ...
            struct stat st1, st2;
            if(EbrStat(path1, &st1) != 0 || EbrStat(path2, &st2) != 0) return false;
