...
@implementation NSNibUnarchiver : NSObject
    ...
    static id constructObject(NSNibUnarchiver* self, Object *pObj)
    {
        if ( strcmp(pObj->className, "NSArray") == 0 || strcmp(pObj->className, "NSMutableArray") == 0 ) {
            ...
            free(values);
            free(keys);
        } else if ( strcmp(pObj->className, "NSString") == 0 || strcmp(pObj->className, "NSMutableString") == 0 || strcmp(pObj->className, "NSLocalizableString") == 0 ) {
            pushObject(self, pObj);
            Item *strContents = itemForKey(self, "NS.bytes");

            if ( strcmp(pObj->className, "NSString") == 0 ) {
                pObj->cachedId = [[[NSString alloc] initWithBytes:strContents->data length:strContents->dataLen encoding:NSUTF8StringEncoding] autorelease];
            } else {
                pObj->cachedId = [[[NSMutableString alloc] initWithBytes:strContents->data length:strContents->dataLen encoding:NSUTF8StringEncoding] autorelease];
            }
            popObject(self);
