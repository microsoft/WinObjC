...
static size_t header_callback(void *ptr, size_t size, size_t nmemb, NSURLProtocol_http *session)
{
    ...
    if ( memcmp(ptr, "HTTP/", 5) == 0 ) {
        ...
        while ( pVersionStart < lineEnd && *pVersionStart != '/' ) pVersionStart ++;
        if ( pVersionStart < lineEnd ) pVersionStart ++;
        char *pVersionEnd = pVersionStart;
        while ( pVersionEnd < lineEnd && *pVersionEnd != ' ' ) pVersionEnd ++;
        char *pStatusEnd = pVersionEnd;
        while ( pStatusEnd < lineEnd && *pStatusEnd != 10 && *pStatusEnd != 13 ) pStatusEnd ++;

        session->_statusLine.attach([[NSString alloc] initWithBytes:pStatusStart length:pStatusEnd - pStatusStart encoding:NSUTF8StringEncoding]);
        session->_statusVersion.attach([[NSString alloc] initWithBytes:pVersionStart length:pVersionEnd - pVersionStart encoding:NSUTF8StringEncoding]);
    } else if ( memchr(ptr, ':', size * nmemb) != NULL ) {
        char *pHeaderStart = (char *) ptr;
        char *pHeaderEnd = pHeaderStart;
        while ( pHeaderEnd < lineEnd && *pHeaderEnd != ':' ) pHeaderEnd ++;
        char *pValueStart = pHeaderEnd + 1;
        while ( pValueStart < lineEnd && *pValueStart == ' ' ) pValueStart ++;
        char *pValueEnd = pValueStart;
        while ( pValueEnd < lineEnd && *pValueEnd != 10 && *pValueEnd != 13 ) pValueEnd ++;
        ...
        while ( curPos < lineEnd && (*curPos == ' ' || *curPos == 13 || *curPos == 10) ) curPos ++;
        ...
        if ( code != 301 && code != 302 && code != 303 && code != 100 ) {           
            id response = [[NSHTTPURLResponse alloc] initWithURL:(id) session->_curURL statusCode:code headers:(id) session->_headers expectedContentLength:(int) contentLength];
...
static size_t readput_callback(void *ptr, size_t size, size_t nmemb, NSURLProtocol_http *session)
{
    ...
    if ( toCopy > session->_putDataLeft ) toCopy = session->_putDataLeft;
...
@implementation NSURLProtocolTimerThread : NSObject
    ...
    {
        if ( self->_easyHandles->size() == 0 ) return;
        if ( self->_queuedRequests->size() == 0 ) return;
            ...
            if ( msg->msg == CURLMSG_DONE ) {
                ...
                if ( msg->data.result == CURLE_OK ) {
                    ...
                    if ( pRedirect != NULL ) {
                        ...
                        id response = [[NSHTTPURLResponse alloc] initWithURL:(id) redirectURL statusCode:code headers:(id) session->_headers expectedContentLength:(int) contentLength];
                        [response setStatusLine:(id) session->_statusLine];
                        [response setStatusVersion:(id) session->_statusVersion];
...
@implementation NSURLProtocol_http
        ...
        for ( ;; ) {
            ...
            switch ( e.type ) {
                case didReceiveResponse:
                    if ( !_hasCancelled ) [_client URLProtocol:self didReceiveResponse:e.obj1 cacheStoragePolicy:NSURLCacheStorageAllowedInMemoryOnly];
                    [e.obj1 autorelease];
                    break;

                case didReceiveData:
                    if ( !_hasCancelled ) [_client URLProtocol:self didLoadData:e.obj1];
                    [e.obj1 release];
                    break;
    ...
    /* annotate with type */ -(id) stopLoading {
        if ( _isFinished ) return self;
        if ( _hasCancelled ) return self;
