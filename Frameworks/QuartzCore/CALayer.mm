...
void DoLayerLayouts(CALayer* window, bool doAlwaysLayers)
{
    ...
    for ( ;; ) {
        ...
        if ( list.curPos == list.count ) break;
...
static void DoDisplayList(CALayer* layer)
{
    ...
    while ( list.curPos < list.count ) {
        ...
        if ( !cur->_textureOverride ) {
            if ( cur->delegate ) EbrDebugLog("Getting new texture for %s\n", object_getClassName(cur->delegate));
            ...
            if ( cur->maskLayer ) {
                ...
                GetCACompositor()->setNodeTexture((DisplayTransaction *) [CATransaction _currentDisplayTransaction], maskLayer->priv->_presentationNode, maskTexture, maskLayer->priv->contentsSize, maskLayer->priv->contentsScale);
                GetCACompositor()->setNodeMaskNode(cur->_presentationNode, maskLayer->priv->_presentationNode);
                if (maskTexture) GetCACompositor()->ReleaseDisplayTexture(maskTexture);
            } 

            GetCACompositor()->setNodeTexture((DisplayTransaction *) [CATransaction _currentDisplayTransaction], cur->_presentationNode, newTexture, cur->contentsSize, cur->contentsScale);
            if ( newTexture ) GetCACompositor()->ReleaseDisplayTexture(newTexture);
        } else {
            cur->needsDisplay = FALSE;
            cur->hasNewContents = FALSE;

            GetCACompositor()->setNodeTexture((DisplayTransaction *) [CATransaction _currentDisplayTransaction], cur->_presentationNode, cur->_textureOverride, cur->contentsSize, cur->contentsScale);
        }
...
@implementation CALayer
    ...
    -(void) display {
        EbrDebugLog("Displaying for 0x%08x (%s, %s)\n", priv->delegate, object_getClassName(self), priv->delegate ? object_getClassName(priv->delegate) : "nil");
        ...
        if ( priv->contentsInset.origin.x != 0.0f ||
             priv->contentsInset.origin.y != 0.0f ||
             priv->contentsInset.size.width != 0.0f ||
    ...
    -(void) removeFromSuperlayer {
        ...
        while ( curLayer != nil ) {
            ...
            if ( curLayer ) nextSuper = curLayer->priv->superlayer;
        }
    ...
    -(void) setPosition:(CGPoint)pos {
        if ( priv->position.x == pos.x && priv->position.y == pos.y ) return;
    ...
    -(void) setBounds:(CGRect)bounds {
        ...
        */
        ...
        if ( priv->bounds.size.width != bounds.size.width ||
             priv->bounds.size.height != bounds.size.height ||
             priv->bounds.origin.x != bounds.origin.x ||
             priv->bounds.origin.y != bounds.origin.y ) {
        ...
        if ( priv->bounds.origin.x != bounds.origin.x ||
             priv->bounds.origin.y != bounds.origin.y ) {
    ...
    -(void) setOrigin:(CGPoint)origin {
        ...
        if ( priv->bounds.origin.x != origin.x ||
             priv->bounds.origin.y != origin.y ) {
            id<CAAction> action = nil;
    ...
    -(void) _releaseContents:(BOOL)immediately {
        ...
        GetCACompositor()->setNodeTexture((DisplayTransaction *) [CATransaction _currentDisplayTransaction], priv->_presentationNode, NULL, CGSizeMake(0, 0), 0.0f);
        priv->needsDisplay = TRUE;
    ...
    -(void) setAffineTransform:(CGAffineTransform)transform {
        ...
        if ( memcmp(priv->transform.m, newTransform.m, sizeof(newTransform.m)) == 0 ) return;
    ...
    -(void) setTransform:(CATransform3D)transform {
        if ( memcmp(priv->transform.m, transform.m, sizeof(transform.m)) == 0 ) return;
    ...
    -(id<CAAction>) actionForKey: (NSString*)key {
        ...
        if ( ret == nil ) {
            ...
            if ( key == _positionAction ) {
                if ( priv->positionSet ) shouldAnimate = true;
            } else if ( key == _boundsOriginAction ) {
                if ( priv->originSet ) shouldAnimate = true;
            } else if ( key == _boundsSizeAction ) {
                if ( priv->sizeSet ) shouldAnimate = true;
    ...
    -(CALayer*) hitTest:(CGPoint)point {
        ...
        curTransform = CATransform3DTranslate(curTransform, -(priv->position.x - priv->bounds.size.width * priv->anchorPoint.x), 
        ...
        if ( point.x >= priv->bounds.origin.x && point.y >= priv->bounds.origin.y &&
             point.x < priv->bounds.origin.x + priv->bounds.size.width && point.y < priv->bounds.origin.x + priv->bounds.size.height ) {
                return self;
        }
    ...
    void GetLayerTransform(CALayer* layer, CGAffineTransform *outTransform)
    {
        ...
        *outTransform = CGAffineTransformMakeTranslation(0.0f, 0.0f);
        ...
        for ( int i = layerListLen - 1; i >= 0; i -- ) {
            ...
            *outTransform = CGAffineTransformTranslate(*outTransform, curLayer->priv->position.x - origin.x, curLayer->priv->position.y - origin.y);
