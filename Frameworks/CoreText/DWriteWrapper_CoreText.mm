...
public:
    CGPathGeometrySink(const CGAffineTransform* transform);

    HRESULT RuntimeClassInitialize();

    void STDMETHODCALLTYPE AddBeziers(_In_ const D2D1_BEZIER_SEGMENT* beziers, unsigned int beziersCount) {
        // Note that both AddBeziers and CGPathAddCurveToPoint use cubic Beziers (rather than quadratic)
    ...
    void STDMETHODCALLTYPE AddLines(_In_ const D2D1_POINT_2F* points, unsigned int pointsCount) {
        ...
        // but that would involve 1) translating the points into CGPoint and storing them in a temporary array first
        // 2) CGPathAddLines does a CGPathMoveToPoint to the first point,
        // which is unwanted as AddLines wants to instead draw from the current point to the first point
        for (unsigned int i = 0; i < pointsCount; ++i) {
            CGPathAddLineToPoint(_cgPath.get(), _transform, points[i].x, -points[i].y);
        }
    }
    ...
    void STDMETHODCALLTYPE BeginFigure(D2D1_POINT_2F startPoint, D2D1_FIGURE_BEGIN figureBegin) {
        ...
        if (!_invalidState) {
            ...
            CGPathMoveToPoint(_cgPath.get(), _transform, startPoint.x, -startPoint.y);
        }
    }
...
CGPathRef _DWriteFontCreatePathForGlyph(const ComPtr<IDWriteFontFace>& fontFace,
    ...
    ComPtr<CGPathGeometrySink> geometrySink = Make<CGPathGeometrySink>(transform);
