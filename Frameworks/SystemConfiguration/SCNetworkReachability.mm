...
@implementation SCNetworkReachability
    ...
    -(void) _performReachabilityTest
    {
        [NSException raiseWithLogging: @"SCNetworkReachabilityInternal" format: @"_performReachabilityTest not implemented on SCNetworkReachability base class"];
    }
    ...
    +(void) _checkGlobalReachability {
        ...
        if ( [internetConnectionProfile internalObject] != nil && [internetConnectionProfile getNetworkConnectivityLevel] == WNCNetworkConnectivityLevelInternetAccess ) {
    ...
    -(void) setCallbackQueue: (dispatch_queue_t) queue
    {
        ...
        if ( queue != NULL ) dispatch_retain(queue);
        if ( _callbackQueue != NULL ) dispatch_release(_callbackQueue);
...
@implementation SCNetworkReachabilityAddress
    ...
    -(void) _performReachabilityTest
    {
        ...
        INT rc = WSAAddressToStringW((LPSOCKADDR) &_address,
        ...
        if ( rc == 0 && addressStrLen > 0 ) {
            ...
            [WNSDatagramSocket getEndpointPairsAsync: wnHostName remoteServiceName: @"0"
