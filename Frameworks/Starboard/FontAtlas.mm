...
public:
    ...
    ~FontCacheInfo()
    {
        ...
        if ( glyphsToBeRendered ) delete[] glyphsToBeRendered;
        [font release];
    ...
    inline int getBitmapWidth() const       { return bitmapWidth; }
    inline int getBitmapHeight() const      { return bitmapHeight; }
...
FontAtlasMan *FontAtlasMan::get()
{
    ...
    if (!impl) impl = new FontAtlasManImpl();
...
bool FontAtlasManImpl::releaseFontRef(id font)
{
    ...
    if ( existing ) {
        ...
        if (existing->refCount) existing->refCount --;
        return true;
    }
...
void FontCacheInfo::setFont(id newFont)
{
    ...
    //renderedSize = pointSize * min(GetCACompositor()->deviceWidth() / GetCACompositor()->screenWidth(), deviceHeight / screenHeight);
    ...
    if ( bitmapHeight < MIN_BITMAP_HEIGHT ) bitmapHeight = MIN_BITMAP_HEIGHT;
...
bool FontCacheInfo::renderNewGlyphs(ID3D11Device1 *device, ID3D11DeviceContext* context)
{
    ...
    for(int curCharIdx = 0; curCharIdx < glyphsToBeRenderedCount; curCharIdx++) {
        ...
        if (rep.mem()) { // some stuff (ie, spaces) don't want to render but we need the metrics.
            ...
            currentTexture->UpdateSubrect(context, startx, starty,
                                          rep.width(), rep.height(), (void*)rep.mem());
        }
    ...
    EbrDebugLog("------ %d glyphs added in %dms. Total %d ops, %d ms.  Atlas chars: %d, size: %dK\n",
                glyphsToBeRenderedCount, ms, totalOps, totalMS,
                (int)[containedChars count], (int)(totalAtlasBytes / 1024));
#endif
...
void DisplayTextureFontAtlas::SetParams(id font, id text, id color, DWORD alignment, DWORD lineBreak, id shadowColor,
...
{
    if (_font != nil) {
        if ( _fontRealized ) FontAtlasMan::get()->releaseFontRef(_font);
        _font = nil;
    }
    _text = nil;
    _color = nil;
    ...
    if ( _outputLines ) EbrFree(_outputLines);
    if ( _atlasGlyphs ) EbrFree(_atlasGlyphs);
    if ( _quadOut ) EbrFree(_quadOut);
    if ( _shadowQuadOut ) EbrFree(_shadowQuadOut);
    if ( _texQuadOut) EbrFree(_texQuadOut);
    if ( _texPtrs ) EbrFree(_texPtrs);
    delete _ellipsis;
}
...
float DisplayTextureFontAtlas::measureWidth(int glyphStart, int numGlyphs)
{
    ...
    for ( int i = glyphStart; i < glyphStart + numGlyphs; i ++ ) {
        if ( !_atlasGlyphs[i].hasSize ) continue;
...
void DisplayTextureFontAtlas::addOutputLine(float x, float y, float maxWidth, int glyphStart, int numGlyphs, CGSize *sizeRet)
{
    ...
    for ( int i = glyphStart + numGlyphs - 1; i >= glyphStart; i -- ) {
        if ( _atlasGlyphs[i].character != ' ' ) break;
        numGlyphs --;
    }
    ...
    for ( ;; ) {
        ...
        if ( _atlasGlyphs[leftChar].hasSize ) {
            if ( curWidth + _atlasGlyphs[leftChar].glyphSize.width > maxWidth - ellipseWidth ) break;
        ...
        if ( _lineBreakMode == UILineBreakModeMiddleTruncation ) {
            ...
            if ( _atlasGlyphs[rightChar - 1].hasSize ) {
                if ( curWidth + _atlasGlyphs[rightChar - 1].glyphSize.width > maxWidth - ellipseWidth ) break;
    ...
    for ( int i = glyphStart; i < glyphStart + numLeft; i ++ ) {
        if ( !_atlasGlyphs[i].hasSize ) continue;
        if ( curWidth + _atlasGlyphs[i].glyphSize.width > maxWidth ) break;
...
void DisplayTextureFontAtlas::emitOperations(CAPoint3D *quads, int numPoints, float alpha, float r, float g, float b,
                                             float a, DrawingObserver *observer, bool sdfFont)
{
    ...
    while ( curPoint < numPoints ) {
        if ( texPtr == NULL ) texPtr = _texPtrs[curPoint];
        if ( texPtr != _texPtrs[curPoint] ) {
            int count = (curPoint - basePoint) / 4;
            _drawingList.AddOperation(&quads[basePoint], &_texQuadOut[basePoint], count, texPtr, alpha, ADM_FONT_BLEND, r / alpha, g / alpha, b / alpha, a, observer, sdfFont);
...
bool DisplayTextureFontAtlas::reflowText(CGSize maxSize, CGSize *totalSizeRet)
{
    ...
    switch ( _lineBreakMode ) {
        ...
        case UILineBreakModeWordWrap: {
            ...
            while ( CGFontWrapFunc(&state, maxSize.width, &fitSize, fontAtlasGetCharWidth, _atlasGlyphs, _lineBreakMode == UILineBreakModeCharacterWrap) ) {
                CGSize retSize;
        ...
        case UILineBreakModeTailTruncation: {
            ...
            while ( CGFontWrapFunc(&state, maxSize.width, &fitSize, fontAtlasGetCharWidth, _atlasGlyphs) ) {
                ...
                if ( curY + (fitSize.height * 2) > maxSize.height + VERTICAL_FORGIVENESS_MARGIN ) {
                    CGSize retSize;
                    addOutputLine(0, curY, maxSize.width, state.lineStart - state.chars, state.chars + state.count - state.lineStart, &retSize);
                    if ( retSize.width > totalSizeRet->width ) {
                        totalSizeRet->width = retSize.width;
                    }
...
void DisplayTextureFontAtlas::addOperation(CADisplayProperties *presentationProperties, CAPoint3D *quad, CGPoint *texquad, int numQuads, float alpha, float r, float g, float b, float a, DrawingObserver *observer)
{
    if ( alpha >= 0.1f ) {
    ...
    if ( _linesDirty ) {
        ...
        if ( _numLines == 1 ) maxSize.height = _lineHeight;
