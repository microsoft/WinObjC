...
#define INTERP(ColourP, ColourQ, ColourR, ColourS, x, y, Result) \
    v = ((y & 0x3) | ((~y & 0x2) << 1)) - BLK_Y_SIZE/2; \
    u = ((x & 0x3) | ((~x & 0x2) << 1)) - BLK_X_4BPP/2; \
    tmp1 = ColourP[0] * 4 + u * (ColourQ[0] - ColourP[0]); \
    tmp2 = (tmp1 * 4 + v * ((ColourR[0] * 4 + u * (ColourS[0] - ColourR[0])) - tmp1)) >> 1; \
    Result[0] = (tmp2 + (tmp2 >> 5)); \
    tmp1 = ColourP[1] * 4 + u * (ColourQ[1] - ColourP[1]); \
    tmp2 = (tmp1 * 4 + v * ((ColourR[1] * 4 + u * (ColourS[1] - ColourR[1])) - tmp1)) >> 1; \
    Result[1] = (tmp2 + (tmp2 >> 5)); \
    tmp1 = ColourP[2] * 4 + u * (ColourQ[2] - ColourP[2]); \
    tmp2 = (tmp1 * 4 + v * ((ColourR[2] * 4 + u * (ColourS[2] - ColourR[2])) - tmp1)) >> 1; \
    Result[2] = (tmp2 + (tmp2 >> 5)); \
    tmp1 = ColourP[3] * 4 + u * (ColourQ[3] - ColourP[3]); \
    tmp2 = tmp1 * 4 + v * ((ColourR[3] * 4 + u * (ColourS[3] - ColourR[3])) - tmp1); \
    Result[3] = (tmp2 + (tmp2 >> 4));

#define INTERP_CHANNEL(channel, ColourP, ColourQ, ColourR, ColourS, x, y, Result, tmpshift, outshift) \
    tmp1 = (ColourP)[channel] * 4 + u * ((ColourQ)[channel] - (ColourP)[channel]); \
    tmp2 = (tmp1 * 4 + v * (((ColourR)[channel] * 4 + u * ((ColourS)[channel] - (ColourR)[channel])) - tmp1)) >> tmpshift; \
    Result = (tmp2 + (tmp2 >> outshift));
...
static inline void InterpolateColours(const int ColourP[4],
                          const int ColourQ[4],
                          const int ColourR[4],
                          const int ColourS[4],
                          const int Do2bitMode,
                          ...
                          int Result[4])
{
    ...
    */
    ...
    if(Do2bitMode)
    {
        u = ((x & 0x7) | ((~x & 0x4) << 1)) - BLK_X_2BPP/2;
        //uscale = 8;

        tmp1 = ColourP[0] * 8 + u * (ColourQ[0] - ColourP[0]);
        tmp2 = (tmp1 * 4 + v * ((ColourR[0] * 8 + u * (ColourS[0] - ColourR[0])) - tmp1)) >> 2;
        Result[0] = (tmp2 + (tmp2 >> 5));
        tmp1 = ColourP[1] * 8 + u * (ColourQ[1] - ColourP[1]);
        tmp2 = (tmp1 * 4 + v * ((ColourR[1] * 8 + u * (ColourS[1] - ColourR[1])) - tmp1)) >> 2;
        Result[1] = (tmp2 + (tmp2 >> 5));
        tmp1 = ColourP[2] * 8 + u * (ColourQ[2] - ColourP[2]);
        tmp2 = (tmp1 * 4 + v * ((ColourR[2] * 8 + u * (ColourS[2] - ColourR[2])) - tmp1)) >> 2;
        Result[2] = (tmp2 + (tmp2 >> 5));
        tmp1 = ColourP[3] * 8 + u * (ColourQ[3] - ColourP[3]);
        tmp2 = (tmp1 * 4 + v * ((ColourR[3] * 8 + u * (ColourS[3] - ColourR[3])) - tmp1)) >> 1;
        Result[3] = (tmp2 + (tmp2 >> 4));
...
#define MODVALUE(xVal, yVal, ModulationVals, ModulationModes, Mod, DoPT) \
{ \
    int x, y; \
    y = (yVal & 0x3) | ((~yVal & 0x2) << 1); \
    x = (xVal & 0x3) | ((~xVal & 0x2) << 1); \
    DoPT = 0; \
    if(ModulationModes[y][x]==0) \
    { \
        Mod = RepVals0[ModulationVals[y][x]]; \
        imgOut[0] = (U8) ((ASig[0] * 8 + Mod * (BSig[0] - ASig[0])) >> 3); \
        imgOut[1] = (U8) ((ASig[1] * 8 + Mod * (BSig[1] - ASig[1])) >> 3); \
        imgOut[2] = (U8) ((ASig[2] * 8 + Mod * (BSig[2] - ASig[2])) >> 3); \
        imgOut[3] = (U8) ((ASig[3] * 8 + Mod * (BSig[3] - ASig[3])) >> 3); \
    } \
    else \
    { \
        Mod = RepVals1[ModulationVals[y][x]]; \
...
#define MODVALUE_MODE(xVal, yVal, ModulationVals, ModulationModes) \
    int modx, mody; \
    int modmode; \
    int Mod; \
    mody = (yVal & 0x3) | ((~yVal & 0x2) << 1); \
    modx = (xVal & 0x3) | ((~xVal & 0x2) << 1); \
    if(ModulationModes[mody][modx]==0) \
    { \
        modmode = 0; \
        Mod = RepVals0[ModulationVals[mody][modx]]; \
    } \
    else \
    { \
        Mod = RepVals1[ModulationVals[mody][modx]]; \
        if ( ModulationVals[mody][modx] != PT_INDEX ) { \
            modmode = 0; \
        } else { \
            modmode = 1; \
        } \
...
#define MODVALUE_APPLY(ASig, BSig) \
    ((U8) ((ASig * 8 + Mod * (BSig - ASig)) >> 3));
...
void DecompressOld(AMTC_BLOCK_STRUCT *pCompressedData,
                const int Do2bitMode,
                const int XDim,
                const int YDim,
                ...
                unsigned char* pResultImage)
{
    ...
    */
    ...
    x = 0;
    y = 0;
    int dy = BLK_Y_SIZE / 2, dx = XBlockSize / 2;

    while ( y < BlkYDim * BLK_Y_SIZE ) {
        x = 0;

        dx = XBlockSize / 2;
        while ( x < BlkXDim * XBlockSize ) {
            /*
            // map this pixel to the top left neighbourhood of blocks
            */
            BlkX = (x - XBlockSize/2);
            ...
            */
            int curx = x, cury = y;
            uPosition = (curx+cury*XDim)<<2;
            unsigned char *imgOut = &pResultImage[uPosition];

            for ( cury = y; cury < y + dy && cury < YDim; cury ++ ) {
                imgOut = &pResultImage[(x+cury*XDim)<<2];

                for ( curx = x; curx < x + dx && curx < XDim; curx ++ ) {
                    if ( !Do2bitMode ) {
                        int u, v;
                        v = ((cury & 0x3) | ((~cury & 0x2) << 1)) - BLK_Y_SIZE/2;
                        u = ((curx & 0x3) | ((~curx & 0x2) << 1)) - BLK_X_4BPP/2;
                        int tmp1, tmp2;
                        int ASigVal, BSigVal;

                        MODVALUE_MODE(curx, cury, ModulationVals, ModulationModes);

                        switch ( modmode ) {
                            case 0:
                                INTERP_CHANNEL(0, Colours5554[0][0].Reps[0], Colours5554[0][1].Reps[0], Colours5554[1][0].Reps[0], Colours5554[1][1].Reps[0],
                                               curx, cury, ASigVal, 1, 5);
                                INTERP_CHANNEL(0, Colours5554[0][0].Reps[1], Colours5554[0][1].Reps[1], Colours5554[1][0].Reps[1], Colours5554[1][1].Reps[1],
                                               curx, cury, BSigVal, 1, 5);
                                imgOut[0] = MODVALUE_APPLY(ASigVal, BSigVal);

                                INTERP_CHANNEL(1, Colours5554[0][0].Reps[0], Colours5554[0][1].Reps[0], Colours5554[1][0].Reps[0], Colours5554[1][1].Reps[0],
                                               curx, cury, ASigVal, 1, 5);
                                INTERP_CHANNEL(1, Colours5554[0][0].Reps[1], Colours5554[0][1].Reps[1], Colours5554[1][0].Reps[1], Colours5554[1][1].Reps[1],
                                               curx, cury, BSigVal, 1, 5);
                                imgOut[1] = MODVALUE_APPLY(ASigVal, BSigVal);

                                INTERP_CHANNEL(2, Colours5554[0][0].Reps[0], Colours5554[0][1].Reps[0], Colours5554[1][0].Reps[0], Colours5554[1][1].Reps[0],
                                               curx, cury, ASigVal, 1, 5);
                                INTERP_CHANNEL(2, Colours5554[0][0].Reps[1], Colours5554[0][1].Reps[1], Colours5554[1][0].Reps[1], Colours5554[1][1].Reps[1],
                                               curx, cury, BSigVal, 1, 5);
                                imgOut[2] = MODVALUE_APPLY(ASigVal, BSigVal);

                                INTERP_CHANNEL(3, Colours5554[0][0].Reps[0], Colours5554[0][1].Reps[0], Colours5554[1][0].Reps[0], Colours5554[1][1].Reps[0],
                                               curx, cury, ASigVal, 0, 4);
                                INTERP_CHANNEL(3, Colours5554[0][0].Reps[1], Colours5554[0][1].Reps[1], Colours5554[1][0].Reps[1], Colours5554[1][1].Reps[1],
                                               curx, cury, BSigVal, 0, 4);
                                imgOut[3] = MODVALUE_APPLY(ASigVal, BSigVal);
                                break;
                        ...
                        InterpolateColours(Colours5554[0][0].Reps[1],
                                           ...
                                           Do2bitMode, curx, cury,
                                           BSig);
                        GetModulationValue(curx, cury, Do2bitMode, (const int (*)[16])ModulationVals, (const int (*)[16])ModulationModes,
                                           &Mod, &DoPT);
