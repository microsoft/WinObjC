...
void OpenGLES20Context::glColor4f(GLfloat red, GLfloat green, GLfloat blue, GLfloat alpha) {
    OpenGLES::Vector4<GLfloat> v4(red, green, blue, alpha);
    openGLESState.setGlobalColor(v4);
}
...
void OpenGLES20Context::glCompressedTexSubImage2D(GLenum target,
                                                  GLint level,
                                                  GLint xoffset,
                                                  GLint yoffset,
                                                  GLsizei width,
...
void OpenGLES20Context::prepareToDraw() {
    ...
    if (openGLESState.isNormal()) {
        ...
        if (openGLESState.isRescaleNormal()) {
            openGLESState.setRescaleNormalFactor(1.0f / sqrtf(modelViewMatrix3x3.m[0] * modelViewMatrix3x3.m[0] +
                                                              modelViewMatrix3x3.m[3] * modelViewMatrix3x3.m[3] +
                                                              modelViewMatrix3x3.m[6] * modelViewMatrix3x3.m[6]));
        }
        ...
        if (openGLESState.isRescaleNormal()) {
        openGLESState.setRescaleNormalFactor(1.0f/sqrtf(transposeInverseModelViewMatrix.m[0]*transposeInverseModelViewMatrix.m[0]
        + transposeInverseModelViewMatrix.m[4]*transposeInverseModelViewMatrix.m[4] +
        transposeInverseModelViewMatrix.m[8]*transposeInverseModelViewMatrix.m[8]));
        }*/
    }
...
void OpenGLES20Context::glTexImage2D(GLenum target,
                                     GLint level,
...
void OpenGLES20Context::glTexSubImage2D(GLenum target,
                                        GLint level,
                                        GLint xoffset,
                                        GLint yoffset,
                                        GLsizei width,
...
void OpenGLES20Context::glClipPlanef(GLenum plane, const GLfloat* equation) {
    ...
    OpenGLESMath::inverse(&tranposeInverseModelViewMatrix,
                          matrixStack.getModelViewMatrix()); // TODO: calculated also before drawing, optimize?
...
void OpenGLES20Context::glGetFloatv(GLenum pname, GLfloat* params) {
    switch (pname) {
        ...
        case GL_TEXTURE_MATRIX:
            for (int i = 0; i < 16; i++) {
                params[i] = matrixStack.getTextureMatrix(openGLESState.getActiveTexture())->m[i];
            }
            break;
        default:
...
void OpenGLES20Context::glVertexAttribPointer(
