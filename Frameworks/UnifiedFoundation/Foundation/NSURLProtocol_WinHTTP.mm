...
#define MakeHttpCompletedCallback ::Microsoft::WRL::Callback<::Microsoft::WRL::Implements<::Microsoft::WRL::RuntimeClassFlags<::Microsoft::WRL::ClassicCom>, IAsyncOperationWithProgressCompletedHandler<HttpResponseMessage*, HttpProgress>>>
#pragma endregion
...
static ComPtr<IHttpRequestMessage> _requestMessageForNS(NSURLRequest* nsRequest) {
    ...
    THROW_IF_FAILED(GetActivationFactory(Wrappers::HStringReference(RuntimeClass_Windows_Web_Http_HttpRequestMessage).Get(), &httpRequestMessageFactory));
    ...
    for (NSString* header in [headers keyEnumerator]) {
        ...
        boolean added = false;
        THROW_IF_FAILED(requestMessageHeaders->TryAppendWithoutValidation(Strings::NarrowToWide<HSTRING>(header).Get(), Strings::NarrowToWide<HSTRING>(value).Get(), &added));
        if (!added) {
    ...
    if (requestBody) {
        ...
        THROW_IF_FAILED(BufferFromRawData(bodyBuffer.GetAddressOf(), reinterpret_cast<unsigned char*>(const_cast<void*>([requestBody bytes])), [requestBody length]));

        ComPtr<IHttpBufferContentFactory> httpBufferContentFactory;
        THROW_IF_FAILED(GetActivationFactory(Wrappers::HStringReference(RuntimeClass_Windows_Web_Http_HttpBufferContent).Get(), &httpBufferContentFactory));
    ...
    if (httpContent && [rejectedRequestHeaders count] > 0) {
        ...
        for (NSString* header in [rejectedRequestHeaders keyEnumerator]) {
            ...
            boolean added = false;
            THROW_IF_FAILED(contentHeaders->TryAppendWithoutValidation(Strings::NarrowToWide<HSTRING>(header).Get(), Strings::NarrowToWide<HSTRING>(value).Get(), &added));
            if (!added) {
...
- (void)startLoading {
    @synchronized (self) {
        ...
        try {
            ...
            THROW_IF_FAILED(_httpClient->SendRequestWithOptionAsync(httpRequestMessage.Get(), HttpCompletionOption::HttpCompletionOption_ResponseHeadersRead, &_httpRequestOperation));
...
- (HRESULT)_asyncHttpOperation:(AsyncHttpOperation*)pOperation completedWithStatus:(AsyncStatus)status {
    ...
    try {
        ...
        if ([_request HTTPShouldHandleCookies]) {
            ...
            HRESULT hr = WRLHelpers::ForEach(headerMap.Get(), [&collectedCookies, &index](const ComPtr<IKeyValuePair<HSTRING, HSTRING>>& pair, boolean* stop) {
                Wrappers::HString key;
                RETURN_IF_FAILED(pair->get_Key(key.GetAddressOf()));
                unsigned int keyLength = 0;
                const wchar_t* rawKey = key.GetRawBuffer(&keyLength);
                if(_wcsnicmp(rawKey, L"set-cookie", _countof("set-cookie") - 1) != 0) { // we want every set-cookie/set-cookie2 header, regardless of case.
                    return S_OK;
                }

                Wrappers::HString value;
                RETURN_IF_FAILED(pair->get_Value(value.GetAddressOf()));
                [collectedCookies setObject:Strings::WideToNSString(value.Get()) forKey:[NSString stringWithFormat:@"%d", ++index]];
                return S_OK;
            });
            THROW_IF_FAILED(hr);
        ...
        NSHTTPURLResponse* nsResponse = [[[NSHTTPURLResponse alloc] initWithURL:[_request URL] statusCode:statusCode headers:nsHeaders expectedContentLength:contentLength] autorelease];
...
- (void)_consumeDataStreamForIHttpContent:(IHttpContent*)pHttpContent {
    ...
    try {
        ...
        WRLHelpers::AwaitProgressCompleteHelper<IInputStream*,uint64_t>([&](IAsyncOperationWithProgress<IInputStream*, uint64_t>** operation) {
        ...
        while(true) {
            ...
            HRESULT hr = WRLHelpers::AwaitProgressCompleteHelper<IBuffer*, uint32_t>([&](IAsyncOperationWithProgress<IBuffer*, uint32_t>** operation) {
