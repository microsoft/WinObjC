...
void appendPath(char *curpath, const char *path)
{
    ...
    if ( strcmp(curpath, "") == 0 ) strcpy_s(curpath, 2048, "/");
}
...
{
    if ( !EbrGetRootMapping(NULL, filePath, 4096) ) return false;
    int curComponent = 0;
    ...
    while ( curToken ) {
        ...
        if ( curComponent == 0 ) {
            if ( !EbrGetRootMapping(curToken, filePath, 4096) ) return false;
...
bool fixPath(char *outPath, const char *relativePath)
{
    ...
    if ( relativePath[0] != '/' ) return false;
...
{
    if ( fixedValid ) return fixedPath;
    return NULL;
}

char *CPathMapper::MappedPath()
{
    if ( fixedValid && mappedValid ) return mappedPath;
    return NULL;
}
...
EbrDirReader* EbrFSDirReader::open(const char* path)
{
    ...
    ret->findHandle = FindFirstFileExW(
        widePath.c_str(),
        FindExInfoStandard,
        &ret->data,
