...
static
void b64_sft(struct mantissa *e1, int n)
/* [<][>][^][v][top][bottom][index][help] */
{
        if (n > 0) {
                if (n > 63) {
...
static
void add_ext(struct EXTEND *e1, struct EXTEND *e2, struct EXTEND *e3)
/* [<][>][^][v][top][bottom][index][help] */
{
...
static PrintType *
_pfloat(long double r, register PrintType *s, int n, int flags)
/* [<][>][^][v][top][bottom][index][help] */
{
...
static PrintType *
_pscien(long double r, register PrintType *s, int n, int flags)
/* [<][>][^][v][top][bottom][index][help] */
{
        int sign, dp; 
        register PrintType *s1;
...
static PrintType *
_gcvt(long double value, int ndigit, PrintType *s, int flags)
/* [<][>][^][v][top][bottom][index][help] */
{
        int sign, dp;
...
PrintType *
_f_print(va_list *ap, int flags, PrintType *s, PrintType c, int precision)
/* [<][>][^][v][top][bottom][index][help] */
{
        register PrintType *old_s = s;
        long double ld_val;

        if (flags & FL_LONGDOUBLE) ld_val = EbrLongDouble(*ap, long double);
        else ld_val = (long double) EbrDouble(*ap, double);
...
PrintType *
_i_compute(unsigned long long val, int base, PrintType *s, int nrdigits)
/* [<][>][^][v][top][bottom][index][help] */
{
        int c;
...
static const PrintType *
gnum(register const PrintType *f, int *ip, va_list *app)
/* [<][>][^][v][top][bottom][index][help] */
{
        register int    i, c;
...
static PrintType *
o_print(va_list *ap, int flags, PrintType *s, PrintType c, int precision, int is_signed)
/* [<][>][^][v][top][bottom][index][help] */
{
...
int
_doprnt(register const PrintType *fmt, va_list ap, PrintOutput *stream)
/* [<][>][^][v][top][bottom][index][help] */
{
        ...
        while (c = *fmt++) {
                ...
                case 'z':       fmt++; break;   //  size_t specifier
                }

                if (*fmt == 'l') {
                    flags |= FL_LONGLONG; fmt ++;
                }
