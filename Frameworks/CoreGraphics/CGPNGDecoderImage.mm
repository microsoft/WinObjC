...
static void PNG_read_data(png_structp png_ptr, png_bytep data, png_size_t length)
{
    ...
    if ( read < length ) memset(data + read, 0, length - read);
}
...
void CGPNGImageBacking::Decode(void *imgDest, int stride)
{
    ...
    if ( imgDest ) {
        ...
        png_get_IHDR (png, info, &png_width, &png_height, &depth, &color_type, &interlace, NULL, NULL);
        if (depth != 8 || ! (color_type == PNG_COLOR_TYPE_RGB || color_type == PNG_COLOR_TYPE_RGB_ALPHA || color_type == PNG_COLOR_TYPE_GRAY || color_type == PNG_COLOR_TYPE_GRAY_ALPHA)) {
            EbrDebugLog("PNG load Error\n");
            errorCode = 1;
            goto error;
        }
...
error:
    if ( errorCode ) {
        ...
        if ( curRow == 0 ) {
            if (row_pointers != NULL) EbrFree(row_pointers);
