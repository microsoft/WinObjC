...
void CGContextImpl::CGContextAddQuadCurveToPoint(float cpx, float cpy, float x, float y)
{
    ...
    CGContextAddCurveToPoint((curr.x/3.0f) + (2.0f*cpx/3.0f), (curr.y/3.0f) + (2.0f*cpy/3.0f),
                                (2.0f*cpx/3.0f) + (x/3.0f), (2.0f*cpy/3.0f) + (y/3.0f),
                                x, y);
...
void CGContextImpl::CGContextAddArcToPoint(float x1, float y1, float x2, float y2, float radius)
{
    ...
    CGContextAddArc((float)(x1 + radius * (t * dx0 + n0x)), (float)(y1 + radius * (t * dy0 + n0y)), radius, (float)atan2(-n0y, -n0x), (float)atan2(-n2y, -n2x), (san < 0));
}
...
void CGContextImpl::CGContextDrawRadialGradient(CGGradientRef gradient, CGPoint startCenter, float startRadius, CGPoint endCenter, float endRadius, DWORD options)
{
}
