...
void CGContextDrawLinearGradient(CGContextRef ctx, CGGradientRef gradient, CGPoint startPoint, CGPoint endPoint, CGGradientDrawingOptions options)
{
    ctx->Backing()->CGContextDrawLinearGradient(gradient, startPoint, endPoint, options);
}

void CGContextDrawRadialGradient(CGContextRef ctx, CGGradientRef gradient, CGPoint startCenter, float startRadius, CGPoint endCenter, float endRadius, CGGradientDrawingOptions options)
{
...
{
    //  We want to center around the cap height. The character will appear at origin.y + ascent, so we need to adjust it so that the capHeight is
    //  centered
