...
template<typename KeyType, typename ValueType, size_t maxHop=30,
    typename Hash1=HashSet1<KeyType>,
    typename Hash2=HashSet2<KeyType> >
class HashMap
{
    struct Bucket
    {
...
public:
    ...
    bool exists(const KeyType& key) const
    {
        ...
        return (_buckets[h1].used && _buckets[h1].key == key) ||
                (_buckets[h2].used && _buckets[h2].key == key);
    }
    ...
    ValueType* insert(KeyType key, ValueType val)
    {
        ...
        for(size_t i = 0; i < maxHop; ++i) {
            ...
            if(curr == _hash1(key) % _size) curr = _hash2(key) % _size;
            else curr = _hash1(key) % _size;
        }
    ...
    {
        ret = handle + 1;
        while(ret < _size && !_buckets[ret].used) ++ret;
        return ret != _size;
    }
...
public:
    ...
    bool exists(const KeyType& key) { return _map.exists(key); }
    void insert(const KeyType& key) { _map.insert(key, None()); }
    void remove(const KeyType& key) { _map.remove(key); }
