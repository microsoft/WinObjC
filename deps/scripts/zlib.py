...
PATCH_FILE = os.path.realpath(os.path.join(".", "zlib.patch"))
...
def compile_project(config):        
    ...
    msbuild_command = [MSBUILD_PATH, "zlib.vcxproj", "/p:configuration=%s" % BUILD_CONFIGURATION]
    if DEBUG:
...
def add_to_prebuilt(output_dir, filename):
    '''copies filename to output_dir'''
    source_file = os.path.join(BUILD_CONFIGURATION, filename)
    shutil.copyfile(source_file, os.path.join(output_dir, filename))
...
def run():
    ...
    zlib_dir = os.path.realpath(os.path.join("..", "3rdParty", "zlib"))
    # apply patch to zlib
    if need_to_apply_patch(zlib_dir):
        result = apply_patch(PATCH_FILE, zlib_dir)
        if not result:
    ...
    for config in configs:
        ...
        result = cmake_generate_project_files(config, zlib_dir)
        if result:
            result = compile_project(config)
            if result:
                output_dir = config.output_directory(OUTPUT_DIR)
                add_to_prebuilt(output_dir, "libz.dll")
                add_to_prebuilt(output_dir, "libz.lib")
                if BUILD_CONFIGURATION in ["Debug", "RelWithDebInfo"]:
                    add_to_prebuilt(output_dir, "libz.pdb")
                continue
                continue
    ...
    add_header(configs[0].output_directory(build_dir), "zconf.h")
    add_header(zlib_dir, "zlib.h")
