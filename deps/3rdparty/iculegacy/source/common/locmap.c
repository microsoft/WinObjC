...
// [jms] Default Windows locale deprecated in Windows Phone.
#if defined(U_WINDOWS) && defined(_MSC_VER) && (_MSC_VER >= 1500) && !defined(_WINPHONE)
...
uprv_convertToPosix(uint32_t hostid, UErrorCode* status)
{
    ...
    int bufferSize = ULOC_FULLNAME_CAPACITY;
    LPCWSTR wideString = malloc(sizeof(wchar_t) * bufferSize);
    ...
    if (ret == 0) {
        ...
        ret = GetLocaleInfoEx(LOCALE_NAME_USER_DEFAULT,
                                ...
                                wideString, 
                                0);
        wideString = malloc(sizeof(wchar_t) * ret);
        ret = GetLocaleInfoEx(LOCALE_NAME_USER_DEFAULT,
                                 LOCALE_SNAME,
                                 wideString, 
                                 bufferSize);
        for (int i = 0; i < bufferSize - 1; ++i) {
            gPosixFromLCID[i] = (char)wideString[i];
        }
        gPosixFromLCID[bufferSize-1] = '\0';
        free(wideString);
    }
    else {
        for (int i = 0; i < ret; ++i) {
