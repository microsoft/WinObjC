...
void __stdcall glReleaseTargets()
{
    EVENT("(glReleaseTargets)");
...
void __stdcall glGetRenderbufferStorageNATIVE(GLvoid **interfaceRet)
{
    gl::Context *context = gl::getNonLostContext();
    GLuint handle = context->getRenderbufferHandle();
	*interfaceRet = context->getRenderbuffer(handle)->getRenderTarget()->getNativeTexture();
}
...
GLvoid * __stdcall glCreateImageANGLE(EGLint width, EGLint height)
{
	gl::Context *context = gl::getNonLostContext();
	gl::Texture2D *texture = context->getTexture2D();
	texture->setImage(0, width, height, GL_RGB, GL_UNSIGNED_BYTE, context->getUnpackAlignment(), NULL);

	return texture;
}

void __stdcall glEGLImageTargetRenderbufferStorageOES(GLenum target, GLeglImageOES image)
{
	gl::Texture2D *texture = (gl::Texture2D *) image;
	gl::Context *context = gl::getNonLostContext();

	context->setRenderbufferStorageTexture2D(texture);
}

void __stdcall glTargetRenderbufferSurface(GLeglImageOES surface)
{
	gl::Context *context = gl::getNonLostContext();

	context->setRenderbufferStorageSwapChain((egl::Surface *) surface);
}
