                                              ...
                                              GLsizei width, GLsizei height, bool mipped)
{
    if (srcTexture)
    {
		...
		/* [blamb: Why are we destroying and re-creating the texture?] */
		/*
        if (!mipped)
        {
...
TextureStorage11_2D::TextureStorage11_2D(Renderer *renderer, int levels, GLenum internalformat, GLenum usage, bool forceRenderable, GLsizei width, GLsizei height, void *directData)
    : TextureStorage11(renderer, GetTextureBindFlags(gl_d3d11::ConvertTextureFormat(internalformat), usage, forceRenderable))
{
    for (unsigned int i = 0; i < gl::IMPLEMENTATION_MAX_TEXTURE_LEVELS; i++)
    {
	...
	*/
    ...
    if (width > 0 && height > 0)
    {
		...
		subData.pSysMem = (const void *) directData;
		subData.SysMemPitch = width * 4;
		subData.SysMemSlicePitch = 0;

        HRESULT result = device->CreateTexture2D(&desc, &subData, &mTexture);
