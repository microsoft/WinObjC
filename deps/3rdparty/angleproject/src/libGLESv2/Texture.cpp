...
{
	if ( format == GL_RGBA && pixels && mTexStorage == NULL && type == GL_UNSIGNED_BYTE ) {
		mDirtyImages = false;
		//mIsImageBackedTexture = true;
        if (mTexStorage)
        {
            delete mTexStorage;
            mTexStorage = NULL;
		...
		mTexStorage = new rx::TextureStorageInterface2D(mRenderer, 1, GL_RGBA8_OES, mUsage, false, width, height, (void *) pixels);
	} else {
		GLint internalformat = ConvertSizedInternalFormat(format, type);
		redefineImage(level, internalformat, width, height);
...
{
	if ( mIsImageBackedTexture ) return;
    if (level < levelCount())
    {
...
{
	if ( mIsImageBackedTexture ) return true;
...
void Texture2D::createTexture()
{
	...
	mDirtyImages = false;
	mIsImageBackedTexture = true;
    mTexStorage = new rx::TextureStorageInterface2D(mRenderer, mImageArray[0], levels, internalformat, mUsage, false, width, height);
#endif
}
