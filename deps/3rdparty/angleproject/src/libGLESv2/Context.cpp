...
void Context::setRenderbufferStorageTexture2D(Texture *texture)
{
	RenderbufferStorage *renderbuffer = new gl::Colorbuffer(texture->getRenderTarget(GL_TEXTURE_2D));
	Renderbuffer *renderbufferObject = mState.renderbuffer.get();
	renderbufferObject->setStorage(renderbuffer);
}

void Context::setRenderbufferStorageSwapChain(egl::Surface *surface)
{
	// Wrap the existing swapchain resources into GL objects and assign them to the '0' names
	rx::SwapChain *swapchain = surface->getSwapChain();

	Colorbuffer *colorbuffer = new Colorbuffer(mRenderer, swapchain);
	Renderbuffer *renderbufferObject = mState.renderbuffer.get();
	renderbufferObject->setStorage(colorbuffer);
}
...
bool Context::unapplyRenderTargets()
{
