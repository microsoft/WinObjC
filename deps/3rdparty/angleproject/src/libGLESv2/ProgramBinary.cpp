                                 ...
                                 FragmentShader *fragmentShader, VertexShader *vertexShader)
{
    ...
    if (shaderModel >= 4 || isWinRT)
    {
        vertexHLSL += "\n"
					  ...
					  "    output.gl_Position.z = (gl_Position.z + gl_Position.w) * 0.5 * dx_ViewAdjust.z;\n"
					  "    output.gl_Position.w = gl_Position.w * dx_ViewAdjust.w;\n";
	}
    else
    {
        vertexHLSL += "\n"
...
bool ProgramBinary::load(InfoLog &infoLog, AttributeBindings &attributeBindings, const void *binary, GLsizei length)
{
	...
	std::vector<Attribute> unlinkedAttributes;
	std::vector<int> unlinkedAttributesIdx;
    ...
    for (int i = 0; i < MAX_VERTEX_ATTRIBS; ++i)
    {
        ...
        if (location != -1)   // Set by glBindAttribLocation
        {
			...
			if ( index != -1 ) {
				unlinkedAttributes.push_back(attribute);
				unlinkedAttributesIdx.push_back(index);
			}
		}
	...
	for ( unsigned i = 0; i < unlinkedAttributes.size(); i ++ ) {
		Attribute attribute = unlinkedAttributes[i];
		...
		mSemanticIndex[availableIndex] = unlinkedAttributesIdx[i];
    }
