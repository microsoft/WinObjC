...
private:
    inline CFTypeRef* _addressof() {
        using TMutable = typename std::add_pointer<typename std::remove_const<typename std::remove_pointer<T>::type>::type>::type;
...
public:
    ...
    // unique_cf used to have inherit semantics on its default constructor.
