...
template<int* destructCounter> struct cxxDestruct {
    ~cxxDestruct() { *destructCounter = *destructCounter + 1; }
};
...
TEST(Objcrt, CxxDestructionHierarchy) {
    ...
    EXPECT_EQ_MSG(destructCounterA, destructCounterLiar, "An instance of CxxDestructLiar should be destructed as many times as its superclass.");
}
