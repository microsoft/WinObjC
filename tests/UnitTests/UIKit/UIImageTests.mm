...
static void testJPEGRepresentationOfCroppedImage(VerifierLambda&& verify) {
    UIImage* photo = [UIImage imageNamed:getPathToFile(@"data/Photo2.jpg")];
    ASSERT_OBJCNE(nil, photo);

    // crop it
    woc::unique_cf<CGImageRef> imageRef{ CGImageCreateWithImageInRect([photo CGImage], CGRectMake(300, 600, 200, 200)) };
    ASSERT_NE(nullptr, imageRef);

    photo = [UIImage imageWithCGImage:imageRef.get()];
    ASSERT_OBJCNE(nil, photo);

    UIImage* croppedRef = [UIImage imageNamed:getPathToFile(@"data/CroppedPhoto2.jpg")];
...
TEST(UIImage, JPEGRepresentation) {
    ...
    testJPEGRepresentationOfCroppedImage(verifier);
